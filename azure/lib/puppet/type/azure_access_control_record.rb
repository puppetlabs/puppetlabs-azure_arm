...
Puppet::Type.newtype(:azure_access_control_record) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_access_control_record you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The properties of access control record."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "The access control record to be added or updated."
    validate do |value|
      true
    end
  end
