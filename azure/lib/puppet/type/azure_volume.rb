...
Puppet::Type.newtype(:azure_volume) do
  @doc = "The volume."
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_volume you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newparam(:parameters) do
    desc "Volume to be created or updated."
    validate do |value|
      true
    end
  end
  ...
  newparam(:volume_container_name) do
    desc "The volume container name."
    validate do |value|
      true
    end
  end
