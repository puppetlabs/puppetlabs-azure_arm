...
Puppet::Type.type(:azure_job_resource).provide(:arm) do
  ...
  def self.fetch_all_as_hash
    ...
    if items
      items.collect do |item|
        ...
        self.deep_delete(hash, [:properties, "details", "copyLogDetails"])
        self.deep_delete(hash, [:properties, "details", "deliveryPackage"])

        self.deep_delete(hash, [:properties, "details", "errorDetails"])
        self.deep_delete(hash, [:properties, "details", "jobStages"])

        self.deep_delete(hash, [:properties, "details", "returnPackage"])
        self.deep_delete(hash, [:properties, "details", "reverseShipmentLabelSasKey"])

        self.deep_delete(hash, [:properties, "error"])
  ...
  def self.invoke_list_with_params(resource = nil, body_params = nil)
    key_values = self.build_key_values
    Puppet.info("Calling operation Jobs_ListByResourceGroup")
    path_params = {}
    query_params = {}
    header_params = {}
    header_params["User-Agent"] = "puppetlabs-azure_arm/0.2.1"
    ...
    self.call_op(path_params, query_params, header_params, body_params, "management.azure.com", "/subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.DataBox/jobs", "Get", "[application/json]")
  end
