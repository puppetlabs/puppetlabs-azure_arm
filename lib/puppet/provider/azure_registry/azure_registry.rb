...
Puppet::Type.type(:azure_registry).provide(:arm) do
  ...
  def self.fetch_all_as_hash
    ...
    if items
      items.collect do |item|
        ...
        self.deep_delete(hash, [:properties, "status", "displayStatus"])
        self.deep_delete(hash, [:properties, "status", "message"])
        self.deep_delete(hash, [:properties, "status", "timestamp"])
  ...
  def self.invoke_list_with_params(resource = nil, body_params = nil)
    key_values = self.build_key_values
    Puppet.info("Calling operation Registries_ListByResourceGroup")
    path_params = {}
    query_params = {}
    header_params = {}
    header_params["User-Agent"] = "puppetlabs-azure_arm/0.2.0"
    ...
    self.call_op(path_params, query_params, header_params, body_params, "management.azure.com", "/subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ContainerRegistry/registries", "Get", "[application/json]")
  end
