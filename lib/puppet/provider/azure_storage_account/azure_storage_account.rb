  ...
  def self.fetch_all_as_hash
    ...
    if items
      items.collect do |item|
        ...
        self.deep_delete(hash, [:properties, "encryption", "services", "blob", "lastEnabledTime"])
        self.deep_delete(hash, [:properties, "failoverInProgress"])

        self.deep_delete(hash, [:properties, "geoReplicationStats", "canFailover"])
        self.deep_delete(hash, [:properties, "geoReplicationStats", "lastSyncTime"])
        self.deep_delete(hash, [:properties, "geoReplicationStats", "status"])
        self.deep_delete(hash, [:properties, "lastGeoFailoverTime"])

        self.deep_delete(hash, [:properties, "primaryEndpoints", "blob"])
        self.deep_delete(hash, [:properties, "primaryEndpoints", "dfs"])
        self.deep_delete(hash, [:properties, "primaryEndpoints", "file"])
        self.deep_delete(hash, [:properties, "primaryEndpoints", "queue"])
        self.deep_delete(hash, [:properties, "primaryEndpoints", "table"])
        self.deep_delete(hash, [:properties, "primaryEndpoints", "web"])
