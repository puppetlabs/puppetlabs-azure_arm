...
Puppet::Type.type(:azure_storage_account).provide(:arm) do
  ...
  def self.fetch_all_as_hash
    ...
    if items
      items.collect do |item|
        ...
        self.deep_delete(hash, [:sku, "capabilities", "name"])
        self.deep_delete(hash, [:sku, "capabilities", "value"])
        self.deep_delete(hash, [:sku, "kind"])
        self.deep_delete(hash, [:sku, "locations"])
        self.deep_delete(hash, [:sku, "resourceType"])

        self.deep_delete(hash, [:sku, "restrictions", "type"])
        self.deep_delete(hash, [:sku, "restrictions", "values"])
  ...
  def self.invoke_create(resource = nil, body_params = nil)
    ...
    op_params = [
      self.op_param("accountName", "path", "account_name"),
      self.op_param("api-version", "query", "api_version"),
