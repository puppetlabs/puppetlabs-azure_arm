...
Puppet::Type.type(:azure_virtual_machine_scale_set_vm).provide(:arm) do
  ...
  def self.invoke_create(resource = nil, body_params = nil)
    ...
    path_params[:instance_id] = resource[:instanceid] unless resource.nil? or resource[:instanceid].nil?
  ...
  def self.invoke_list_with_params(resource = nil, body_params = nil)
    ...
    path_params[:subscription_id] = ENV["azure_subscription_id"] unless ENV["azure_subscription_id"].nil?
    path_params[:subscription_id] = resource[:subscription_id] unless resource.nil? or resource[:subscription_id].nil?
    path_params[:virtual_machine_scale_set_name] = key_values["virtualMachineScaleSetName"] unless key_values["virtualMachineScaleSetName"].nil?
    path_params[:virtual_machine_scale_set_name] = ENV["azure_virtual_machine_scale_set_name"] unless ENV["azure_virtual_machine_scale_set_name"].nil?
    path_params[:virtual_machine_scale_set_name] = resource[:virtual_machine_scale_set_name] unless resource.nil? or resource[:virtual_machine_scale_set_name].nil?
    uri_string = "https://management.azure.com/subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Compute/virtualMachineScaleSets/%{virtual_machine_scale_set_name}/virtualMachines" % path_params
    uri_string = uri_string + "?" + to_query(query_params)
    header_params['Content-Type'] = 'application/json' # first of [application/json]
    if authenticate(path_params, query_params, header_params, body_params)
      Puppet.info("Authentication succeeded")
