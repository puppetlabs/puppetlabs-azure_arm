...
Puppet::Type.type(:azure_workbook).provide(:arm) do
  ...
  def self.invoke_create(resource = nil, body_params = nil)
    ...
    op_params = [
      ...
      self.op_param("workbookProperties", "body", "properties"),
    ]
    op_params.each do |i|
      inquery = i[:inquery]
      name = i[:name]
  ...
  def self.invoke_list_with_params(resource = nil, body_params = nil)
    ...
    op_params = [
      self.op_param("api-version", "query", "api_version"),
      self.op_param("canFetchContent", "query", "can_fetch_content"),
      self.op_param("category", "query", "category"),
      self.op_param("resourceGroupName", "path", "resource_group_name"),
      self.op_param("subscriptionId", "path", "subscription_id"),
      self.op_param("tags", "query", "tags"),
    ]
    op_params.each do |i|
      inquery = i[:inquery]
      name = i[:name]
