...
Puppet::Type.newtype(:azure_agent_pool) do
  @doc = "Agent Pool."
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_agent_pool you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Properties of an agent pool."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Parameters supplied to the Create or Update an agent pool operation."
    validate do |value|
      true
    end
  end
