...
Puppet::Type.newtype(:azure_app_service_certificate_resource) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_app_service_certificate_resource you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newparam(:key_vault_certificate) do
    desc "Key vault certificate resource Id."
    validate do |value|
      true
    end
  end
