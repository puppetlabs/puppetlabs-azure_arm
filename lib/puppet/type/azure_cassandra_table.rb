...
Puppet::Type.newtype(:azure_cassandra_table) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_cassandra_table you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The properties of an Azure Cosmos DB Cassandra table"
    validate do |value|
      true
    end
  end
  ...
  newparam(:create_update_cassandra_table_parameters) do
    desc "The parameters to provide for the current Cassandra Table."
    validate do |value|
      true
    end
  end
