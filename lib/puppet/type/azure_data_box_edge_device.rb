...
Puppet::Type.newtype(:azure_data_box_edge_device) do
  ...
  newproperty(:location) do
    desc "The location of the device. This is a supported and registered Azure geographical region (for example, West US, East US, or Southeast Asia). The geographical region of a device cannot be changed once it is created, but if an identical geographical region is specified on update, the request will succeed."
    validate do |value|
      true
    end
  end
  ...
  newproperty(:tags) do
    desc "The list of tags that describe the device. These tags can be used to view and group this device (across resource groups)."
    validate do |value|
      true
    end
  end
