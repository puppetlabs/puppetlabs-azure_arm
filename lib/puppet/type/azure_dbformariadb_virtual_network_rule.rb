...
Puppet::Type.newtype(:azure_dbformariadb_virtual_network_rule) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_dbformariadb_virtual_network_rule you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
