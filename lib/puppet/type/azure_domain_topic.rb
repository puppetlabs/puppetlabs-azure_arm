...
Puppet::Type.newtype(:azure_domain_topic) do
  @doc = "Domain Topic."
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_domain_topic you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Properties of the Domain Topic."
    validate do |value|
      true
    end
  end
