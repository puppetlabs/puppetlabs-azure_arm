...
Puppet::Type.newtype(:azure_domainregistration_domain) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_domainregistration_domain you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Domain resource specific properties"
    validate do |value|
      true
    end
  end
