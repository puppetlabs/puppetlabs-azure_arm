...
Puppet::Type.newtype(:azure_environment_setting) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_environment_setting you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The properties of the Environment Setting resource"
    validate do |value|
      true
    end
  end
  ...
  newparam(:environment_setting) do
    desc "Represents settings of an environment, from which environment instances would be created"
    validate do |value|
      true
    end
  end
