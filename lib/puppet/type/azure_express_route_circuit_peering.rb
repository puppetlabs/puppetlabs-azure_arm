...
Puppet::Type.newtype(:azure_express_route_circuit_peering) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_express_route_circuit_peering you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the express route circuit peering."
    validate do |value|
      true
    end
  end
  ...
  newparam(:peering_parameters) do
    desc "Parameters supplied to the create or update express route circuit peering operation."
    validate do |value|
      true
    end
  end
