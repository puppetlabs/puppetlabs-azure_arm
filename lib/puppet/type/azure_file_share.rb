...
Puppet::Type.newtype(:azure_file_share) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_file_share you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the file share."
    validate do |value|
      true
    end
  end
  ...
  newparam(:file_share) do
    desc "Properties of the file share to create."
    validate do |value|
      true
    end
  end
  ...
  newparam(:share_name) do
    desc "The name of the file share within the specified storage account. File share names must be between 3 and 63 characters in length and use numbers, lower-case letters and dash (-) only. Every dash (-) character must be immediately preceded and followed by a letter or number."
    validate do |value|
      true
    end
  end
