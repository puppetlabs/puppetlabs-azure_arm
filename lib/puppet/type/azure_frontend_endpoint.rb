...
Puppet::Type.newtype(:azure_frontend_endpoint) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_frontend_endpoint you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Properties of the Frontend endpoint"
    validate do |value|
      true
    end
  end
  ...
  newparam(:frontend_endpoint_parameters) do
    desc "Frontend endpoint properties needed to create a new endpoint."
    validate do |value|
      true
    end
  end
