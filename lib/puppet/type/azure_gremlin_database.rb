...
Puppet::Type.newtype(:azure_gremlin_database) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_gremlin_database you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The properties of an Azure Cosmos DB SQL database"
    validate do |value|
      true
    end
  end
  ...
  newparam(:create_update_gremlin_database_parameters) do
    desc "The parameters to provide for the current Gremlin database."
    validate do |value|
      true
    end
  end
