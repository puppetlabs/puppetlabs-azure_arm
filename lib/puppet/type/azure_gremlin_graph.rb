...
Puppet::Type.newtype(:azure_gremlin_graph) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_gremlin_graph you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The properties of an Azure Cosmos DB Gremlin graph"
    validate do |value|
      true
    end
  end
  ...
  newparam(:create_update_gremlin_graph_parameters) do
    desc "The parameters to provide for the current Gremlin graph."
    validate do |value|
      true
    end
  end
  ...
  newparam(:graph_name) do
    desc "Cosmos DB graph name."
    validate do |value|
      true
    end
  end
