...
Puppet::Type.newtype(:azure_hybrid_connection) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_hybrid_connection you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Properties of the HybridConnection."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Parameters supplied to create a hybrid connection."
    validate do |value|
      true
    end
  end
