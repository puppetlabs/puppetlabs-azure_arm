...
Puppet::Type.newtype(:azure_inbound_nat_rule) do
  ...
  newparam(:name) do
    isnamevar
    desc "Gets name of the resource that is unique within the set of inbound NAT rules used by the load balancer. This name can be used to access the resource."
    validate do |value|
      true
    end
  end
