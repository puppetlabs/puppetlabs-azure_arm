...
Puppet::Type.newtype(:azure_issue_comment_contract) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_issue_comment_contract you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Properties of the Issue Comment."
    validate do |value|
      true
    end
  end
  ...
  newparam(:comment_id) do
    desc "Comment identifier within an Issue. Must be unique in the current Issue."
    validate do |value|
      true
    end
  end
