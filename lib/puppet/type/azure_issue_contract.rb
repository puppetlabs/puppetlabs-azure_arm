...
Puppet::Type.newtype(:azure_issue_contract) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_issue_contract you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
