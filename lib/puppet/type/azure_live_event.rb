...
Puppet::Type.newtype(:azure_live_event) do
  @doc = "The Live Event."
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_live_event you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The Live Event properties."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Live Event properties needed for creation."
    validate do |value|
      true
    end
  end
