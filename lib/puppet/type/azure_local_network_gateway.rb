...
Puppet::Type.newtype(:azure_local_network_gateway) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_local_network_gateway you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the local network gateway."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Parameters supplied to the create or update local network gateway operation."
    validate do |value|
      true
    end
  end
