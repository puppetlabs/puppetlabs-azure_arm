...
Puppet::Type.newtype(:azure_log_search_rule_resource) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_log_search_rule_resource you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The rule properties of the resource."
    validate do |value|
      true
    end
  end
