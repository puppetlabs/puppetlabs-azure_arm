...
Puppet::Type.newtype(:azure_migration_item) do
  @doc = "Migration item."
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_migration_item you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The migration item properties."
    validate do |value|
      true
    end
  end
