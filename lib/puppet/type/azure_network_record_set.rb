...
Puppet::Type.newtype(:azure_network_record_set) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_network_record_set you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newparam(:private_zone_name) do
    desc "The name of the Private DNS zone (without a terminating dot)."
    validate do |value|
      true
    end
  end
  newparam(:record_type) do
    desc "The type of DNS record in this record set. Record sets of type SOA can be updated but not created (they are created when the Private DNS zone is created)."
    validate do |value|
      true
    end
  end
