...
Puppet::Type.newtype(:azure_network_security_group) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_network_security_group you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the network security group."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Parameters supplied to the create or update network security group operation."
    validate do |value|
      true
    end
  end
