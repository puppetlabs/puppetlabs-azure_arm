...
Puppet::Type.newtype(:azure_network_watcher) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_network_watcher you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the network watcher."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Parameters that define the network watcher resource."
    validate do |value|
      true
    end
  end
