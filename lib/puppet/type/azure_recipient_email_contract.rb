...
Puppet::Type.newtype(:azure_recipient_email_contract) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_recipient_email_contract you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Recipient Email contract properties."
    validate do |value|
      true
    end
  end
