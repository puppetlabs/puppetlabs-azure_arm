...
Puppet::Type.newtype(:azure_replication) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_replication you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The properties of the replication."
    validate do |value|
      true
    end
  end
  ...
  newparam(:replication) do
    desc "The parameters for creating a replication."
    validate do |value|
      true
    end
  end
