...
Puppet::Type.newtype(:azure_route_filter) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_route_filter you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the route filter."
    validate do |value|
      true
    end
  end
  ...
  newparam(:route_filter_parameters) do
    desc "Parameters supplied to the create or update route filter operation."
    validate do |value|
      true
    end
  end
