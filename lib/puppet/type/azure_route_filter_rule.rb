...
Puppet::Type.newtype(:azure_route_filter_rule) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_route_filter_rule you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newproperty(:properties) do
    desc "Properties of the route filter rule."
    validate do |value|
      true
    end
  end
  ...
  newparam(:route_filter_name) do
    desc "The name of the route filter."
    validate do |value|
      true
    end
  end
  newparam(:route_filter_rule_parameters) do
    desc "Parameters supplied to the create or update route filter rule operation."
    validate do |value|
      true
    end
  end
  newparam(:rule_name) do
    desc "The name of the route filter rule."
    validate do |value|
      true
    end
  end
