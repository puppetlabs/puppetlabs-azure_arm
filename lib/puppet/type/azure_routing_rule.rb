...
Puppet::Type.newtype(:azure_routing_rule) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_routing_rule you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "Properties of the Front Door Routing Rule"
    validate do |value|
      true
    end
  end
  ...
  newparam(:routing_rule_parameters) do
    desc "Routing Rule properties needed to create a new Front Door."
    validate do |value|
      true
    end
  end
