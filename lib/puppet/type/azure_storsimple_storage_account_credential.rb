...
Puppet::Type.newtype(:azure_storsimple_storage_account_credential) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_storsimple_storage_account_credential you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newparam(:parameters) do
    desc "The storage account credential to be added or updated."
    validate do |value|
      true
    end
  end
