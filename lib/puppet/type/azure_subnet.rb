...
Puppet::Type.newtype(:azure_subnet) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_subnet you must provide a value for #{property}"
      end
    end
  end
  newproperty(:etag) do
  ...
  newparam(:subnet_parameters) do
    desc "Parameters supplied to the create or update subnet operation."
    validate do |value|
      true
    end
  end
