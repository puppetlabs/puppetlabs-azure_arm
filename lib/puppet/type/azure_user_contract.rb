...
Puppet::Type.newtype(:azure_user_contract) do
  @doc = "User details."
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_user_contract you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "User entity contract properties."
    validate do |value|
      true
    end
  end
  ...
  newparam(:if_match) do
    desc "The entity state (Etag) version of the user to delete. A value of '*' can be used for If-Match to unconditionally apply the operation."
    validate do |value|
      true
    end
  end
