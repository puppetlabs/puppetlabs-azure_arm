...
Puppet::Type.newtype(:azure_virtual_machine) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_virtual_machine you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:identity) do
    desc "The identity of the virtual machine, if configured."
    validate do |value|
      true
    end
  end
  ...
  newproperty(:zones, :array_matching => :all) do
    desc "The virtual machine zones."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "Parameters supplied to the Create Virtual Machine operation."
    validate do |value|
      true
    end
  end
