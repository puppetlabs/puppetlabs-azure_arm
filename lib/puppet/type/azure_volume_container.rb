...
Puppet::Type.newtype(:azure_volume_container) do
  ...
  validate do
    ...
    required_properties.each do |property|
      ...
      if self[:ensure] == :present && self[property].nil? && self.provider.send(property) == :absent
        raise Puppet::Error, "In azure_volume_container you must provide a value for #{property}"
      end
    end
  end
  newproperty(:id) do
  ...
  newproperty(:properties) do
    desc "The volume container properties."
    validate do |value|
      true
    end
  end
  ...
  newparam(:parameters) do
    desc "The volume container to be added or updated."
    validate do |value|
      true
    end
  end
