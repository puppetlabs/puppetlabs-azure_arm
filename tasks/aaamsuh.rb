...
def api_management_service_update_hostname(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ApiManagement/service/%{service_name}/updatehostname" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('delete', 'body', 'delete', 'delete'),
      ...
      op_param('update', 'body', 'update', 'update'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = api_management_service_update_hostname(params)
