...
def app_service_environments_create_or_update_worker_pool(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/hostingEnvironments/%{name}/workerPools/%{worker_pool_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('sku', 'body', 'sku', 'sku'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('type', 'body', 'type', 'type'),
      op_param('workerPoolEnvelope', 'body', 'worker_pool_envelope', 'worker_pool_envelope'),
      op_param('workerPoolName', 'path', 'worker_pool_name', 'worker_pool_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = app_service_environments_create_or_update_worker_pool(params)
