...
def adds_services_user_preference_add(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//providers/Microsoft.ADHybridHealthService/addsservices/%{service_name}/features/%{feature_name}/userpreference" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('featureName', 'path', 'feature_name', 'feature_name'),
      op_param('metricnames', 'body', 'metric_names', 'metricnames'),
      op_param('serviceName', 'path', 'service_name', 'service_name'),
      op_param('setting', 'body', 'setting', 'setting'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = adds_services_user_preference_add(params)
