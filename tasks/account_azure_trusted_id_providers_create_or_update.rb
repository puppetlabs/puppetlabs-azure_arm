...
def trusted_id_providers_create_or_update(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('trustedIdProviderName', 'path', 'trusted_id_provider_name', 'trusted_id_provider_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = trusted_id_providers_create_or_update(params)
