...
def virtual_network_rules_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.DataLakeStore/accounts/%{account_name}/virtualNetworkRules/%{virtual_network_rule_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('virtualNetworkRuleName', 'path', 'virtual_network_rule_name', 'virtual_network_rule_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_network_rules_create_or_update(params)
