...
def service_get_metrics(*args)
  ...
  uri_string = "https://management.azure.com//providers/Microsoft.ADHybridHealthService/services/%{service_name}/metrics/%{metric_name}/groups/%{group_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('fromDate', 'query', 'from_date', 'from_date'),
      op_param('groupKey', 'query', 'group_key', 'group_key'),
      op_param('groupName', 'path', 'group_name', 'group_name'),
      op_param('metricName', 'path', 'metric_name', 'metric_name'),
      op_param('serviceName', 'path', 'service_name', 'service_name'),
      op_param('sets', 'body', 'sets', 'sets'),
      op_param('timestamps', 'body', 'time_stamps', 'timestamps'),
      op_param('toDate', 'query', 'to_date', 'to_date'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = service_get_metrics(params)
