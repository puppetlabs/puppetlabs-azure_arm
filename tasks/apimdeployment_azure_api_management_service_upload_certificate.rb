...
def api_management_service_upload_certificate(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ApiManagement/service/%{service_name}/updatecertificate" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('expiry', 'body', 'expiry', 'expiry'),
      ...
      op_param('subject', 'body', 'subject', 'subject'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('thumbprint', 'body', 'thumbprint', 'thumbprint'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = api_management_service_upload_certificate(params)
