...
def tag_description_create_or_update(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('apiId', 'path', 'api_id', 'api_id'),
      ...
      op_param('tagId', 'path', 'tag_id', 'tag_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = tag_description_create_or_update(params)
