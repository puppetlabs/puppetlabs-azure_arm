...
def autoscale_settings_get(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('autoscaleSettingName', 'path', 'autoscale_setting_name', 'autoscale_setting_name'),
      op_param('enabled', 'body', 'enabled', 'enabled'),
      op_param('name', 'body', 'name', 'name'),
      op_param('notifications', 'body', 'notifications', 'notifications'),
      ...
      op_param('targetresourceuri', 'body', 'targetresourceuri', 'targetresourceuri'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = autoscale_settings_get(params)
