...
def metric_baseline_get(*args)
  ...
  uri_string = "https://management.azure.com//%{resource_uri}/providers/microsoft.insights/baseline/%{metric_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('aggregation', 'query', 'aggregation', 'aggregation'),
      op_param('api-version', 'query', 'api_version', 'api_version'),
      op_param('highthresholds', 'body', 'high_thresholds', 'highthresholds'),
      op_param('interval', 'query', 'interval', 'interval'),
      op_param('lowthresholds', 'body', 'low_thresholds', 'lowthresholds'),
      op_param('metricName', 'path', 'metric_name', 'metric_name'),
      op_param('metricnamespace', 'query', 'metricnamespace', 'metricnamespace'),
      op_param('resourceUri', 'path', 'resource_uri', 'resource_uri'),
      op_param('resultType', 'query', 'result_type', 'result_type'),
      op_param('sensitivities', 'query', 'sensitivities', 'sensitivities'),
      op_param('sensitivity', 'body', 'sensitivity', 'sensitivity'),
      op_param('timespan', 'query', 'timespan', 'timespan'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = metric_baseline_get(params)
