...
def jobs_list_output_files(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.BatchAI/workspaces/%{workspace_name}/experiments/%{experiment_name}/jobs/%{job_name}/listOutputFiles" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('directory', 'query', 'directory', 'directory'),
      op_param('error', 'body', 'error', 'error'),
      op_param('experimentName', 'path', 'experiment_name', 'experiment_name'),
      op_param('jobName', 'path', 'job_name', 'job_name'),
      op_param('linkexpiryinminutes', 'query', 'linkexpiryinminutes', 'linkexpiryinminutes'),
      op_param('maxresults', 'query', 'maxresults', 'maxresults'),
      op_param('outputdirectoryid', 'query', 'outputdirectoryid', 'outputdirectoryid'),
...
def task
  ...
  params = STDIN.read
  result = jobs_list_output_files(params)
