...
def metric_baseline_calculate_baseline(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//%{resource_uri}/providers/microsoft.insights/calculatebaseline" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('TimeSeriesInformation', 'body', 'time_series_information', 'time_series_information'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = metric_baseline_calculate_baseline(params)
