...
def container_execute_command(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ContainerInstance/containerGroups/%{container_group_name}/containers/%{container_name}/exec" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('containerExecRequest', 'body', 'container_exec_request', 'container_exec_request'),
      op_param('containerGroupName', 'path', 'container_group_name', 'container_group_name'),
      ...
      op_param('websocketuri', 'body', 'web_socket_uri', 'websocketuri'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = container_execute_command(params)
