...
def database_accounts_failover_priority_change(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.DocumentDB/databaseAccounts/%{account_name}/failoverPriorityChange" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('accountName', 'path', 'failover_policies', 'account_name'),
      op_param('api-version', 'query', 'failover_policies', 'api_version'),
      op_param('failoverParameters', 'body', 'failover_policies', 'failover_parameters'),
      op_param('failoverpolicies', 'body', 'failover_policies', 'failoverpolicies'),
      op_param('resourceGroupName', 'path', 'failover_policies', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'failover_policies', 'subscription_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = database_accounts_failover_priority_change(params)
