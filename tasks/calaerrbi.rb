...
def log_analytics_export_request_rate_by_interval(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.Compute/locations/%{location}/logAnalytics/apiAccess/getRequestRateByInterval" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('blobcontainersasuri', 'body', 'blob_container_sas_uri', 'blobcontainersasuri'),
      op_param('fromtime', 'body', 'from_time', 'fromtime'),
      op_param('groupbyoperationname', 'body', 'group_by_operation_name', 'groupbyoperationname'),
      op_param('groupbyresourcename', 'body', 'group_by_resource_name', 'groupbyresourcename'),
      op_param('groupbythrottlepolicy', 'body', 'group_by_throttle_policy', 'groupbythrottlepolicy'),
      op_param('intervallength', 'body', 'interval_length', 'intervallength'),
      op_param('location', 'path', 'location', 'location'),
      op_param('parameters', 'body', 'parameters', 'parameters'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('totime', 'body', 'to_time', 'totime'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = log_analytics_export_request_rate_by_interval(params)
