...
def list_management_image_add_image(*args)
  header_params = {}
  ...
  uri_string = "https:////contentmoderator/lists/v1.0/imagelists/%{list_id}/images" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('label', 'query', 'label', 'label'),
      op_param('listId', 'path', 'list_id', 'list_id'),
      op_param('tag', 'query', 'tag', 'tag'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = list_management_image_add_image(params)
