...
def predictions_get_model_status(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/predictions/%{prediction_name}/getModelStatus" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('modelversion', 'body', 'modelversion', 'modelversion'),
      op_param('prediction_name', 'path', 'prediction_name', 'prediction_name'),
      op_param('predictionguidid', 'body', 'predictionguidid', 'predictionguidid'),
      ...
      op_param('signalsused', 'body', 'signalsused', 'signalsused'),
      op_param('status', 'body', 'status', 'status'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('tenantid', 'body', 'tenantid', 'tenantid'),
      op_param('testsetcount', 'body', 'testsetcount', 'testsetcount'),
      op_param('trainingaccuracy', 'body', 'trainingaccuracy', 'trainingaccuracy'),
      op_param('trainingsetcount', 'body', 'trainingsetcount', 'trainingsetcount'),
      op_param('validationsetcount', 'body', 'validationsetcount', 'validationsetcount'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = predictions_get_model_status(params)
