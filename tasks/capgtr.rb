...
def predictions_get_training_results(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/predictions/%{prediction_name}/getTrainingResults" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('canonicalprofiles', 'body', 'canonical_profiles', 'canonicalprofiles'),
      op_param('hubName', 'path', 'hub_name', 'hub_name'),
      op_param('predictiondistribution', 'body', 'prediction_distribution', 'predictiondistribution'),
      op_param('predictionName', 'path', 'prediction_name', 'prediction_name'),
      op_param('primaryprofileinstancecount', 'body', 'primary_profile_instance_count', 'primaryprofileinstancecount'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('scorename', 'body', 'score_name', 'scorename'),
...
def task
  ...
  params = STDIN.read
  result = predictions_get_training_results(params)
