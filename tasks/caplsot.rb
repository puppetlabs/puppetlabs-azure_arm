...
def profiles_list_supported_optimization_types(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Cdn/profiles/%{profile_name}/getSupportedOptimizationTypes" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('supportedoptimizationtypes', 'body', 'supported_optimization_types', 'supportedoptimizationtypes'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = profiles_list_supported_optimization_types(params)
