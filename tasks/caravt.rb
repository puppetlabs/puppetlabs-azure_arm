...
def reviews_add_video_transcript(*args)
  header_params = {}
  ...
  uri_string = "https:////contentmoderator/review/v1.0/teams/%{team_name}/reviews/%{review_id}/transcript" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('VTT file', 'body', 'vtt file', 'vtt file'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = reviews_add_video_transcript(params)
