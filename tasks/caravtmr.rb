...
def reviews_add_video_transcript_moderation_result(*args)
  header_params = {}
  ...
  uri_string = "https:////contentmoderator/review/v1.0/teams/%{team_name}/reviews/%{review_id}/transcriptmoderationresult" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('reviewId', 'path', 'review_id', 'review_id'),
      op_param('teamName', 'path', 'team_name', 'team_name'),
      op_param('transcriptModerationBody', 'body', 'transcript_moderation_body', 'transcript_moderation_body'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = reviews_add_video_transcript_moderation_result(params)
