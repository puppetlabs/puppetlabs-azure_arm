...
def registries_update_policies(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ContainerRegistry/registries/%{registry_name}/updatePolicies" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('quarantinepolicy', 'body', 'quarantine_policy', 'quarantinepolicy'),
      op_param('registryName', 'path', 'registry_name', 'registry_name'),
      op_param('registryPoliciesUpdateParameters', 'body', 'registry_policies_update_parameters', 'registry_policies_update_parameters'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('trustpolicy', 'body', 'trust_policy', 'trustpolicy'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = registries_update_policies(params)
