...
def catalog_delete_credential(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('cascade', 'query', 'cascade', 'cascade'),
      op_param('credentialName', 'path', 'credential_name', 'credential_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_DeleteCredential(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_delete_credential(params)
