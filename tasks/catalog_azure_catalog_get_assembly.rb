...
def catalog_get_assembly(*args)
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/assemblies/%{assembly_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('assembly_name', 'path', 'assembly_name', 'assembly_name'),
      op_param('assemblyname', 'body', 'assembly_name', 'assemblyname'),
      op_param('clrname', 'body', 'clr_name', 'clrname'),
      ...
      op_param('dependencies', 'body', 'dependencies', 'dependencies'),
      op_param('files', 'body', 'files', 'files'),
      op_param('isuserdefined', 'body', 'is_user_defined', 'isuserdefined'),
      op_param('isvisible', 'body', 'is_visible', 'isvisible'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetAssembly(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_assembly(params)
