...
def catalog_get_credential(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/credentials/%{credential_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('credential_name', 'path', 'credential_name', 'credential_name'),
      op_param('credentialname', 'body', 'credential_name', 'credentialname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetCredential(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_credential(params)
