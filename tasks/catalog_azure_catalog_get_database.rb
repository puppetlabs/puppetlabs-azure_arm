...
def catalog_get_database(*args)
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}" % path_params
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetDatabase(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_database(params)
