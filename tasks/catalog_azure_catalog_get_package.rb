...
def catalog_get_package(*args)
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/packages/%{package_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('package_name', 'path', 'package_name', 'package_name'),
      op_param('packagename', 'body', 'package_name', 'packagename'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetPackage(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_package(params)
