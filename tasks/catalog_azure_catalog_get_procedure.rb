...
def catalog_get_procedure(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/procedures/%{procedure_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('procedureName', 'path', 'procedure_name', 'procedure_name'),
      op_param('procname', 'body', 'proc_name', 'procname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetProcedure(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_procedure(params)
