...
def catalog_get_secret(*args)
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/secrets/%{secret_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('secret_name', 'path', 'secret_name', 'secret_name'),
      op_param('secretname', 'body', 'secret_name', 'secretname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetSecret(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_secret(params)
