...
def catalog_get_table_partition(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tables/%{table_name}/partitions/%{partition_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('createdate', 'body', 'create_date', 'createdate'),
      op_param('database_name', 'path', 'database_name', 'database_name'),
      op_param('databasename', 'body', 'database_name', 'databasename'),
      op_param('indexid', 'body', 'index_id', 'indexid'),
      op_param('label', 'body', 'label', 'label'),
      op_param('parentname', 'body', 'parent_name', 'parentname'),
      op_param('partition_name', 'path', 'partition_name', 'partition_name'),
      op_param('partitionname', 'body', 'partition_name', 'partitionname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetTablePartition(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_table_partition(params)
