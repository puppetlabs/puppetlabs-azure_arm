...
def catalog_get_table_statistic(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tables/%{table_name}/statistics/%{statistics_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('colnames', 'body', 'col_names', 'colnames'),
      op_param('computeaccountname', 'body', 'compute_account_name', 'computeaccountname'),
      op_param('createtime', 'body', 'create_time', 'createtime'),
      op_param('database_name', 'path', 'database_name', 'database_name'),
      op_param('databasename', 'body', 'database_name', 'databasename'),
      op_param('filterdefinition', 'body', 'filter_definition', 'filterdefinition'),
      op_param('hasfilter', 'body', 'has_filter', 'hasfilter'),
      op_param('isautocreated', 'body', 'is_auto_created', 'isautocreated'),
      op_param('isusercreated', 'body', 'is_user_created', 'isusercreated'),
      op_param('schema_name', 'path', 'schema_name', 'schema_name'),
      op_param('schemaname', 'body', 'schema_name', 'schemaname'),
      op_param('statdatapath', 'body', 'stat_data_path', 'statdatapath'),
      op_param('statistics_name', 'path', 'statistics_name', 'statistics_name'),
      op_param('statisticsname', 'body', 'statistics_name', 'statisticsname'),
      op_param('table_name', 'path', 'table_name', 'table_name'),
      op_param('tablename', 'body', 'table_name', 'tablename'),
      op_param('updatetime', 'body', 'update_time', 'updatetime'),
      op_param('userstatname', 'body', 'user_stat_name', 'userstatname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetTableStatistic(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_table_statistic(params)
