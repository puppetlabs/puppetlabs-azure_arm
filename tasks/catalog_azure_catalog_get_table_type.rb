...
def catalog_get_table_type(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tabletypes/%{table_type_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('columns', 'body', 'columns', 'columns'),
      op_param('computeaccountname', 'body', 'compute_account_name', 'computeaccountname'),
      op_param('csharpname', 'body', 'c_sharp_name', 'csharpname'),
      op_param('database_name', 'path', 'database_name', 'database_name'),
      op_param('databasename', 'body', 'database_name', 'databasename'),
      op_param('fullcsharpname', 'body', 'full_c_sharp_name', 'fullcsharpname'),
      op_param('isassemblytype', 'body', 'is_assembly_type', 'isassemblytype'),
      op_param('iscomplextype', 'body', 'is_complex_type', 'iscomplextype'),
      op_param('isnullable', 'body', 'is_nullable', 'isnullable'),
      op_param('istabletype', 'body', 'is_table_type', 'istabletype'),
      op_param('isuserdefined', 'body', 'is_user_defined', 'isuserdefined'),
      op_param('principalid', 'body', 'principal_id', 'principalid'),
      op_param('schema_name', 'path', 'schema_name', 'schema_name'),
      op_param('schemaid', 'body', 'schema_id', 'schemaid'),
      op_param('schemaname', 'body', 'schema_name', 'schemaname'),
      op_param('systemtypeid', 'body', 'system_type_id', 'systemtypeid'),
      op_param('tableTypeName', 'path', 'table_type_name', 'table_type_name'),
      op_param('typefamily', 'body', 'type_family', 'typefamily'),
      op_param('typename', 'body', 'type_name', 'typename'),
      op_param('usertypeid', 'body', 'user_type_id', 'usertypeid'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetTableType(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_table_type(params)
