...
def catalog_get_table_valued_function(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tablevaluedfunctions/%{table_valued_function_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('tableValuedFunctionName', 'path', 'table_valued_function_name', 'table_valued_function_name'),
      op_param('tvfname', 'body', 'tvf_name', 'tvfname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_GetTableValuedFunction(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_get_table_valued_function(params)
