...
def catalog_list_assemblies(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/assemblies" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('assemblyclrname', 'body', 'assembly_clr_name', 'assemblyclrname'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_ListAssemblies(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_list_assemblies(params)
