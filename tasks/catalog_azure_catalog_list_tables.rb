...
def catalog_list_tables(*args)
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tables" % path_params
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_ListTables(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_list_tables(params)
