...
def catalog_preview_table(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tables/%{table_name}/previewrows" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('totalcolumncount', 'body', 'totalcolumncount', 'totalcolumncount'),
      op_param('totalrowcount', 'body', 'totalrowcount', 'totalrowcount'),
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_PreviewTable(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_preview_table(params)
