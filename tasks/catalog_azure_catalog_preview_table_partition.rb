...
def catalog_preview_table_partition(*args)
  header_params = {}
  ...
  uri_string = "https:////catalog/usql/databases/%{database_name}/schemas/%{schema_name}/tables/%{table_name}/partitions/%{partition_name}/previewrows" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('maxColumns', 'query', 'max_columns', 'max_columns'),
      op_param('maxRows', 'query', 'max_rows', 'max_rows'),
      op_param('partitionName', 'path', 'partition_name', 'partition_name'),
      op_param('rows', 'body', 'rows', 'rows'),
      op_param('schema', 'body', 'schema', 'schema'),
      op_param('schemaName', 'path', 'schema_name', 'schema_name'),
      op_param('tableName', 'path', 'table_name', 'table_name'),
      op_param('totalcolumncount', 'body', 'total_column_count', 'totalcolumncount'),
      op_param('totalrowcount', 'body', 'total_row_count', 'totalrowcount'),
      op_param('truncated', 'body', 'truncated', 'truncated'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Catalog_PreviewTablePartition(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = catalog_preview_table_partition(params)
