...
def virtual_machine_scale_sets_force_recovery_service_fabric_platform_update_domain_walk(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Compute/virtualMachineScaleSets/%{vm_scale_set_name}/forceRecoveryServiceFabricPlatformUpdateDomainWalk" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('nextplatformupdatedomain', 'body', 'next_platform_update_domain', 'nextplatformupdatedomain'),
      op_param('platformUpdateDomain', 'query', 'platform_update_domain', 'platform_update_domain'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('vmScaleSetName', 'path', 'vm_scale_set_name', 'vm_scale_set_name'),
      op_param('walkperformed', 'body', 'walk_performed', 'walkperformed'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_machine_scale_sets_force_recovery_service_fabric_platform_update_domain_walk(params)
