...
def registries_get_build_source_upload_url(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ContainerRegistry/registries/%{registry_name}/listBuildSourceUploadUrl" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('uploadurl', 'body', 'upload_url', 'uploadurl'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = registries_get_build_source_upload_url(params)
