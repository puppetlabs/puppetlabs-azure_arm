...
def virtual_machine_images_get(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.Compute/locations/%{location}/publishers/%{publisher_name}/artifacttypes/vmimage/offers/%{offer}/skus/%{skus}/versions/%{version}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('offer', 'path', 'offer', 'offer'),
      op_param('properties', 'body', 'properties', 'properties'),
      op_param('publisherName', 'path', 'publisher_name', 'publisher_name'),
      op_param('skus', 'path', 'skus', 'skus'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('tags', 'body', 'tags', 'tags'),
      op_param('version', 'path', 'version', 'version'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_machine_images_get(params)
