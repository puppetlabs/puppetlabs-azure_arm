...
def virtual_machine_scale_set_v_ms_reimage(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Compute/virtualMachineScaleSets/%{vm_scale_set_name}/virtualmachines/%{instance_id}/reimage" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('instanceId', 'path', 'instance_id', 'instance_id'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('tempdisk', 'body', 'temp_disk', 'tempdisk'),
      op_param('vmScaleSetName', 'path', 'vm_scale_set_name', 'vm_scale_set_name'),
      op_param('vmScaleSetVMReimageInput', 'body', 'vm_scale_set_vm_reimage_input', 'vm_scale_set_vm_reimage_input'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_machine_scale_set_v_ms_reimage(params)
