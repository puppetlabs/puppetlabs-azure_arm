...
def virtual_machines_instance_view(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Compute/virtualMachines/%{vm_name}/instanceView" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('identity', 'body', 'identity', 'identity'),
      op_param('location', 'body', 'location', 'location'),
      op_param('name', 'body', 'name', 'name'),
      op_param('plan', 'body', 'plan', 'plan'),
      ...
      op_param('zones', 'body', 'zones', 'zones'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_machines_instance_view(params)
