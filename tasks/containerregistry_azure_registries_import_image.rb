...
def registries_import_image(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ContainerRegistry/registries/%{registry_name}/importImage" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('targettags', 'body', 'target_tags', 'targettags'),
      op_param('untaggedtargetrepositories', 'body', 'untagged_target_repositories', 'untaggedtargetrepositories'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = registries_import_image(params)
