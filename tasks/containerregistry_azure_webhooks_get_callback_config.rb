...
def webhooks_get_callback_config(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ContainerRegistry/registries/%{registry_name}/webhooks/%{webhook_name}/getCallbackConfig" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('customheaders', 'body', 'custom_headers', 'customheaders'),
      ...
      op_param('serviceuri', 'body', 'service_uri', 'serviceuri'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('webhookName', 'path', 'webhook_name', 'webhook_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = webhooks_get_callback_config(params)
