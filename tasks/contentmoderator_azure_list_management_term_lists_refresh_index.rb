...
def list_management_term_lists_refresh_index(*args)
  header_params = {}
  ...
  uri_string = "https:////contentmoderator/lists/v1.0/termlists/%{list_id}/RefreshIndex" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('advancedinfo', 'body', 'advancedinfo', 'advancedinfo'),
      op_param('contentsourceid', 'body', 'contentsourceid', 'contentsourceid'),
      op_param('isupdatesuccess', 'body', 'isupdatesuccess', 'isupdatesuccess'),
      op_param('language', 'query', 'language', 'language'),
      op_param('listId', 'path', 'list_id', 'list_id'),
      op_param('status', 'body', 'status', 'status'),
      op_param('trackingid', 'body', 'trackingid', 'trackingid'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = list_management_term_lists_refresh_index(params)
