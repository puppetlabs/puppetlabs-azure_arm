...
def reviews_create_reviews(*args)
  header_params = {}
  ...
  uri_string = "https:////contentmoderator/review/v1.0/teams/%{team_name}/reviews" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('createReviewBody', 'body', 'create_review_body', 'create_review_body'),
      op_param('error', 'body', 'error', 'error'),
      op_param('subTeam', 'query', 'sub_team', 'sub_team'),
      op_param('teamName', 'path', 'team_name', 'team_name'),
      op_param('UrlContentType', 'header', 'url_content_type', 'url_content_type'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = reviews_create_reviews(params)
