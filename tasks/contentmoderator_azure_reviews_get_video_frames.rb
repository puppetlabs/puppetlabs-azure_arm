...
def reviews_get_video_frames(*args)
  header_params = {}
  ...
  uri_string = "https:////contentmoderator/review/v1.0/teams/%{team_name}/reviews/%{review_id}/frames" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('noOfRecords', 'query', 'no_of_records', 'no_of_records'),
      op_param('reviewId', 'path', 'review_id', 'review_id'),
      op_param('startSeed', 'query', 'start_seed', 'start_seed'),
...
def task
  ...
  params = STDIN.read
  result = reviews_get_video_frames(params)
