...
def collection_region_list_metrics(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.DocumentDB/databaseAccounts/%{account_name}/region/%{region}/databases/%{database_rid}/collections/%{collection_rid}/metrics" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('collectionRid', 'path', 'collection_rid', 'collection_rid'),
      op_param('databaseRid', 'path', 'database_rid', 'database_rid'),
      op_param('region', 'path', 'region', 'region'),
...
def task
  ...
  params = STDIN.read
  result = collection_region_list_metrics(params)
