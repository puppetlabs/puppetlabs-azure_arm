...
def interactions_get(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/interactions/%{interaction_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('interactionName', 'path', 'interaction_name', 'interaction_name'),
      op_param('locale-code', 'query', 'locale_code', 'locale_code'),
...
def task
  ...
  params = STDIN.read
  result = interactions_get(params)
