...
def interactions_suggest_relationship_links(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/interactions/%{interaction_name}/suggestRelationshipLinks" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('interaction_name', 'path', 'interaction_name', 'interaction_name'),
      op_param('interactionname', 'body', 'interaction_name', 'interactionname'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('suggestedrelationships', 'body', 'suggested_relationships', 'suggestedrelationships'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = interactions_suggest_relationship_links(params)
