...
def predictions_model_status(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/predictions/%{prediction_name}/modelStatus" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('modelversion', 'body', 'model_version', 'modelversion'),
      op_param('parameters', 'body', 'parameters', 'parameters'),
      op_param('prediction_name', 'path', 'prediction_name', 'prediction_name'),
      op_param('predictionguidid', 'body', 'prediction_guid_id', 'predictionguidid'),
      op_param('predictionname', 'body', 'prediction_name', 'predictionname'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('signalsused', 'body', 'signals_used', 'signalsused'),
      op_param('status', 'body', 'status', 'status'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('tenantid', 'body', 'tenant_id', 'tenantid'),
      op_param('testsetcount', 'body', 'test_set_count', 'testsetcount'),
      op_param('trainingaccuracy', 'body', 'training_accuracy', 'trainingaccuracy'),
      op_param('trainingsetcount', 'body', 'training_set_count', 'trainingsetcount'),
      op_param('validationsetcount', 'body', 'validation_set_count', 'validationsetcount'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = predictions_model_status(params)
