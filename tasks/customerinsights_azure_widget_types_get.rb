...
def widget_types_get(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/widgetTypes/%{widget_type_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('changed', 'body', 'changed', 'changed'),
      ...
      op_param('imageurl', 'body', 'image_url', 'imageurl'),
      ...
      op_param('widget_type_name', 'path', 'widget_type_name', 'widget_type_name'),
      op_param('widgettypename', 'body', 'widget_type_name', 'widgettypename'),
      op_param('widgetversion', 'body', 'widget_version', 'widgetversion'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = widget_types_get(params)
