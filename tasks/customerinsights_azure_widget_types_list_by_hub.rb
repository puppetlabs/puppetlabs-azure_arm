...
def widget_types_list_by_hub(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.CustomerInsights/hubs/%{hub_name}/widgetTypes" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('imageurl', 'body', 'imageurl', 'imageurl'),
      ...
      op_param('widgettypename', 'body', 'widgettypename', 'widgettypename'),
      op_param('widgetversion', 'body', 'widgetversion', 'widgetversion'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = widget_types_list_by_hub(params)
