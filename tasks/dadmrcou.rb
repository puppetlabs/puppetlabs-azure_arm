...
def data_masking_rules_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Sql/servers/%{server_name}/databases/%{database_name}/dataMaskingPolicies/%{data_masking_policy_name}/rules/%{data_masking_rule_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('dataMaskingPolicyName', 'path', 'data_masking_policy_name', 'data_masking_policy_name'),
      op_param('dataMaskingRuleName', 'path', 'name', 'data_masking_rule_name'),
...
def task
  ...
  params = STDIN.read
  result = data_masking_rules_create_or_update(params)
