...
def path_list(*args)
  ...
  uri_string = "https:////%{filesystem}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('continuation', 'query', 'continuation', 'continuation'),
      op_param('directory', 'query', 'directory', 'directory'),
      op_param('filesystem', 'path', 'filesystem', 'filesystem'),
      op_param('maxResults', 'query', 'max_results', 'max_results'),
      op_param('paths', 'body', 'paths', 'paths'),
      op_param('recursive', 'query', 'recursive', 'recursive'),
      op_param('resource', 'query', 'resource', 'resource'),
      op_param('timeout', 'query', 'timeout', 'timeout'),
      op_param('upn', 'query', 'upn', 'upn'),
      op_param('x-ms-client-request-id', 'header', 'x_ms_client_request_id', 'x_ms_client_request_id'),
      op_param('x-ms-date', 'header', 'x_ms_date', 'x_ms_date'),
      op_param('x-ms-version', 'header', 'x_ms_version', 'x_ms_version'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
