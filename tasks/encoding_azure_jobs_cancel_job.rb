...
def jobs_cancel_job(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Media/mediaServices/%{account_name}/transforms/%{transform_name}/jobs/%{job_name}/cancelJob" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('transformName', 'path', 'transform_name', 'transform_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
