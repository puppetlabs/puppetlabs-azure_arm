...
def express_route_cross_connections_list_routes_table_summary(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/expressRouteCrossConnections/%{cross_connection_name}/peerings/%{peering_name}/routeTablesSummary/%{device_path}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('crossConnectionName', 'path', 'cross_connection_name', 'cross_connection_name'),
      op_param('devicePath', 'path', 'device_path', 'device_path'),
      op_param('nextlink', 'body', 'next_link', 'nextlink'),
      op_param('peeringName', 'path', 'peering_name', 'peering_name'),
...
def task
  ...
  params = STDIN.read
  result = express_route_cross_connections_list_routes_table_summary(params)
