...
def express_route_links_get(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/ExpressRoutePorts/%{express_route_port_name}/links/%{link_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('expressRoutePortName', 'path', 'express_route_port_name', 'express_route_port_name'),
      op_param('id', 'body', 'id', 'id'),
      op_param('linkName', 'path', 'link_name', 'link_name'),
...
def task
  ...
  params = STDIN.read
  result = express_route_links_get(params)
