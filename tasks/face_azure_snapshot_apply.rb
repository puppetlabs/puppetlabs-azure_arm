...
def snapshot_apply(*args)
  ...
  uri_string = "https:////snapshots/%{snapshot_id}/apply" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('snapshotId', 'path', 'snapshot_id', 'snapshot_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
