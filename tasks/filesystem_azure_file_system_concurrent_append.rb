...
def file_system_concurrent_append(*args)
  header_params = {}
  ...
  uri_string = "https:////WebHdfsExt/%{path}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('appendMode', 'query', 'append_mode', 'append_mode'),
      op_param('op', 'query', 'op', 'op'),
      op_param('path', 'path', 'path', 'path'),
      op_param('remoteexception', 'body', 'remote_exception', 'remoteexception'),
      op_param('streamContents', 'body', 'stream_contents', 'stream_contents'),
      op_param('syncFlag', 'query', 'sync_flag', 'sync_flag'),
      op_param('Transfer-Encoding', 'header', 'transfer_encoding', 'transfer_encoding'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = FileSystem_ConcurrentAppend(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = file_system_concurrent_append(params)
