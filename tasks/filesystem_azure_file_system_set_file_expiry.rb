...
def file_system_set_file_expiry(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('expireTime', 'query', 'expire_time', 'expire_time'),
      op_param('expiryOption', 'query', 'expiry_option', 'expiry_option'),
...
def authenticate
  ...
  params = STDIN.read
  result = FileSystem_SetFileExpiry(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = file_system_set_file_expiry(params)
