...
def o_auth2_permission_grant_create(*args)
  header_params = {}
  ...
  uri_string = "https://graph.windows.net//%{tenant_id}/oauth2PermissionGrants" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('consenttype', 'body', 'consent_type', 'consenttype'),
      op_param('expirytime', 'body', 'expiry_time', 'expirytime'),
      op_param('objectid', 'body', 'object_id', 'objectid'),
      op_param('odata.type', 'body', 'odata.type', 'odata.type'),
...
def task
  ...
  params = STDIN.read
  result = o_auth2_permission_grant_create(params)
