...
def objects_get_objects_by_object_ids(*args)
  header_params = {}
  ...
  uri_string = "https://graph.windows.net//%{tenant_id}/getObjectsByObjectIds" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('includedirectoryobjectreferences', 'body', 'include_directory_object_references', 'includedirectoryobjectreferences'),
      op_param('objectids', 'body', 'object_ids', 'objectids'),
      op_param('parameters', 'body', 'parameters', 'parameters'),
      op_param('tenantID', 'path', 'tenant_id', 'tenant_id'),
      op_param('types', 'body', 'types', 'types'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = objects_get_objects_by_object_ids(params)
