...
def attested_get_document(*args)
  header_params = {}
  ...
  uri_string = "https://169.254.169.254//attested/document" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('encoding', 'body', 'encoding', 'encoding'),
      op_param('Metadata', 'header', 'metadata', 'metadata'),
      op_param('nonce', 'query', 'nonce', 'nonce'),
...
def authenticate
  ...
  params = STDIN.read
  result = Attested_GetDocument(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = attested_get_document(params)
