...
def identity_get_token(*args)
  ...
  uri_string = "https://169.254.169.254//identity/oauth2/token" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('access_token', 'body', 'access_token', 'access_token'),
      op_param('api-version', 'query', 'api_version', 'api_version'),
      op_param('authority', 'query', 'authority', 'authority'),
      op_param('bypass_cache', 'query', 'bypass_cache', 'bypass_cache'),
      op_param('client_id', 'body', 'client_id', 'client_id'),
      op_param('expires_in', 'body', 'expires_in', 'expires_in'),
      op_param('expires_on', 'body', 'expires_on', 'expires_on'),
      op_param('ext_expires_in', 'body', 'ext_expires_in', 'ext_expires_in'),
      op_param('Metadata', 'header', 'metadata', 'metadata'),
      op_param('msi_res_id', 'body', 'msi_res_id', 'msi_res_id'),
      op_param('not_before', 'body', 'not_before', 'not_before'),
      op_param('object_id', 'body', 'object_id', 'object_id'),
      op_param('resource', 'body', 'resource', 'resource'),
      op_param('token_type', 'body', 'token_type', 'token_type'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Identity_GetToken(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = identity_get_token(params)
