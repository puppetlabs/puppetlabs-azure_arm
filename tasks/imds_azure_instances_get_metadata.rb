...
def instances_get_metadata(*args)
  header_params = {}
  ...
  uri_string = "https://169.254.169.254//instance" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('compute', 'body', 'compute', 'compute'),
      op_param('Metadata', 'header', 'metadata', 'metadata'),
      op_param('network', 'body', 'network', 'network'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Instances_GetMetadata(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = instances_get_metadata(params)
