...
def job_get_debug_data_path(*args)
  header_params = {}
  ...
  uri_string = "https:////Jobs/%{job_identity}/GetDebugDataPath" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('command', 'body', 'command', 'command'),
...
def authenticate
  ...
  params = STDIN.read
  result = Job_GetDebugDataPath(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = job_get_debug_data_path(params)
