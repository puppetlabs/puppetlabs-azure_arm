...
def job_get_statistics(*args)
  ...
  uri_string = "https:////Jobs/%{job_identity}/GetStatistics" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('finalizingtimeutc', 'body', 'finalizing_time_utc', 'finalizingtimeutc'),
      op_param('jobIdentity', 'path', 'job_identity', 'job_identity'),
      op_param('lastupdatetimeutc', 'body', 'last_update_time_utc', 'lastupdatetimeutc'),
      op_param('stages', 'body', 'stages', 'stages'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Job_GetStatistics(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = job_get_statistics(params)
