...
def get_certificate_operation(*args)
  header_params = {}
  ...
  uri_string = "https:////certificates/%{certificate_name}/pending" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('cancellation_requested', 'body', 'cancellation_requested', 'cancellation_requested'),
      op_param('certificate-name', 'path', 'certificate_name', 'certificate_name'),
      op_param('csr', 'body', 'csr', 'csr'),
      op_param('error', 'body', 'error', 'error'),
      op_param('id', 'body', 'id', 'id'),
      op_param('issuer', 'body', 'issuer', 'issuer'),
      op_param('request_id', 'body', 'request_id', 'request_id'),
      op_param('status', 'body', 'status', 'status'),
      op_param('status_details', 'body', 'status_details', 'status_details'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetCertificateOperation(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_certificate_operation(params)
