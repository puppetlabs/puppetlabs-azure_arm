...
def get_deleted_storage_account(*args)
  header_params = {}
  ...
  uri_string = "https:////deletedstorage/%{storage_account_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('activekeyname', 'body', 'active_key_name', 'activekeyname'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetDeletedStorageAccount(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_deleted_storage_account(params)
