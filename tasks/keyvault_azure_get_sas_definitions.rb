...
def get_sas_definitions(*args)
  ...
  uri_string = "https:////storage/%{storage_account_name}/sas" % path_params
...
def authenticate
  ...
  params = STDIN.read
  result = GetSasDefinitions(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_sas_definitions(params)
