...
def recover_deleted_certificate(*args)
  header_params = {}
  ...
  uri_string = "https:////deletedcertificates/%{certificate_name}/recover" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('cer', 'body', 'cer', 'cer'),
      op_param('certificate-name', 'path', 'certificate_name', 'certificate_name'),
      op_param('contenttype', 'body', 'contenttype', 'contenttype'),
...
def authenticate
  ...
  params = STDIN.read
  result = RecoverDeletedCertificate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = recover_deleted_certificate(params)
