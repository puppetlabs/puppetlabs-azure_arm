...
def recover_deleted_sas_definition(*args)
  header_params = {}
  ...
  uri_string = "https:////deletedstorage/%{storage_account_name}/sas/%{sas_definition_name}/recover" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('sastype', 'body', 'sastype', 'sastype'),
      op_param('sid', 'body', 'sid', 'sid'),
      op_param('storage-account-name', 'path', 'storage_account_name', 'storage_account_name'),
      op_param('tags', 'body', 'tags', 'tags'),
      op_param('templateuri', 'body', 'templateuri', 'templateuri'),
      op_param('validityperiod', 'body', 'validityperiod', 'validityperiod'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = RecoverDeletedSasDefinition(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = recover_deleted_sas_definition(params)
