...
def set_certificate_issuer(*args)
  header_params = {}
  ...
  uri_string = "https:////certificates/issuers/%{issuer_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('issuer-name', 'path', 'issuer_name', 'issuer_name'),
      op_param('org_details', 'body', 'org_details', 'org_details'),
      op_param('parameter', 'body', 'parameter', 'parameter'),
...
def authenticate
  ...
  params = STDIN.read
  result = SetCertificateIssuer(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = set_certificate_issuer(params)
