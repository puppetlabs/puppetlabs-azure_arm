...
def vaults_check_name_availability(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.KeyVault/checkNameAvailability" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('vaultName', 'body', 'vault_name', 'vault_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = vaults_check_name_availability(params)
