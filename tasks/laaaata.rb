...
def azure_accounts_assign_to_app(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/%{app_id}/azureaccounts" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('accountname', 'body', 'account_name', 'accountname'),
      op_param('appId', 'path', 'app_id', 'app_id'),
      op_param('Authorization', 'header', 'authorization', 'authorization'),
      op_param('azureAccountInfoObject', 'body', 'azure_account_info_object', 'azure_account_info_object'),
      op_param('azuresubscriptionid', 'body', 'azure_subscription_id', 'azuresubscriptionid'),
...
def task
  ...
  params = STDIN.read
  result = azure_accounts_assign_to_app(params)
