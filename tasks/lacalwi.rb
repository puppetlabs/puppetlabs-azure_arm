...
def custom_apis_list_wsdl_interfaces(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.Web/locations/%{location}/listWsdlInterfaces" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('wsdlDefinition', 'body', 'wsdl_definition', 'wsdl_definition'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = custom_apis_list_wsdl_interfaces(params)
