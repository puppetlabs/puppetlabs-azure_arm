...
def model_update_hierarchical_entity(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/%{app_id}/versions/%{version_id}/hierarchicalentities/%{h_entity_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('children', 'body', 'children', 'children'),
      op_param('hEntityId', 'path', 'h_entity_id', 'h_entity_id'),
      op_param('hierarchicalModelUpdateObject', 'body', 'hierarchical_model_update_object', 'hierarchical_model_update_object'),
...
def task
  ...
  params = STDIN.read
  result = model_update_hierarchical_entity(params)
