...
def model_update_hierarchical_entity_child(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/%{app_id}/versions/%{version_id}/hierarchicalentities/%{h_entity_id}/children/%{h_child_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('hChildId', 'path', 'h_child_id', 'h_child_id'),
      op_param('hEntityId', 'path', 'h_entity_id', 'h_entity_id'),
      op_param('hierarchicalChildModelUpdateObject', 'body', 'hierarchical_child_model_update_object', 'hierarchical_child_model_update_object'),
...
def task
  ...
  params = STDIN.read
  result = model_update_hierarchical_entity_child(params)
