...
def integration_accounts_log_tracking_events(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Logic/integrationAccounts/%{integration_account_name}/logTrackingEvents" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('events', 'body', 'events', 'events'),
      op_param('integrationAccountName', 'path', 'integration_account_name', 'integration_account_name'),
      op_param('logTrackingEvents', 'body', 'log_tracking_events', 'log_tracking_events'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('sourcetype', 'body', 'source_type', 'sourcetype'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('trackeventsoptions', 'body', 'track_events_options', 'trackeventsoptions'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = integration_accounts_log_tracking_events(params)
