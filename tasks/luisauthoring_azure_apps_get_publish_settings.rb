...
def apps_get_publish_settings(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('speech', 'body', 'speech', 'speech'),
      op_param('spellchecker', 'body', 'spell_checker', 'spellchecker'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = apps_get_publish_settings(params)
