...
def apps_list_cortana_endpoints(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/assistants" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('endpointkeys', 'body', 'endpoint_keys', 'endpointkeys'),
      op_param('endpointurls', 'body', 'endpoint_urls', 'endpointurls'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = apps_list_cortana_endpoints(params)
