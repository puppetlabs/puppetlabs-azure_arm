...
def apps_update_publish_settings(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/%{app_id}/publishsettings" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('publishSettingUpdateObject', 'body', 'publish_setting_update_object', 'publish_setting_update_object'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = apps_update_publish_settings(params)
