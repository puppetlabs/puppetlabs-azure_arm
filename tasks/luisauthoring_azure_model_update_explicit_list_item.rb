...
def model_update_explicit_list_item(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/%{app_id}/versions/%{version_id}/patternanyentities/%{entity_id}/explicitlist/%{item_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('explicitlistitem', 'body', 'explicit_list_item', 'explicitlistitem'),
      op_param('item', 'body', 'item', 'item'),
      op_param('itemId', 'path', 'item_id', 'item_id'),
...
def task
  ...
  params = STDIN.read
  result = model_update_explicit_list_item(params)
