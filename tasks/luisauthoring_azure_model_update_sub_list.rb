...
def model_update_sub_list(*args)
  header_params = {}
  ...
  uri_string = "https:////apps/%{app_id}/versions/%{version_id}/closedlists/%{cl_entity_id}/sublists/%{sub_list_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('subListId', 'path', 'sub_list_id', 'sub_list_id'),
      op_param('versionId', 'path', 'version_id', 'version_id'),
      op_param('wordListBaseUpdateObject', 'body', 'word_list_base_update_object', 'word_list_base_update_object'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = model_update_sub_list(params)
