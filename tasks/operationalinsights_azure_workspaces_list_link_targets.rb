...
def workspaces_list_link_targets(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.OperationalInsights/linkTargets" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('customerid', 'body', 'customer_id', 'customerid'),
      op_param('location', 'body', 'location', 'location'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('workspacename', 'body', 'workspace_name', 'workspacename'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = workspaces_list_link_targets(params)
