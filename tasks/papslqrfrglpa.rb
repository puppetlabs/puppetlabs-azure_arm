...
def policy_states_list_query_results_for_resource_group_level_policy_assignment(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourcegroups/%{resource_group_name}/providers/%{authorization_namespace}/policyAssignments/%{policy_assignment_name}/providers/Microsoft.PolicyInsights/policyStates/%{policy_states_resource}/queryResults" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('$apply', 'query', '$apply', '$apply'),
      op_param('$filter', 'query', '$filter', '$filter'),
      op_param('$from', 'query', '$from', '$from'),
      op_param('$orderby', 'query', '$orderby', '$orderby'),
      op_param('$select', 'query', '$select', '$select'),
      op_param('$to', 'query', '$to', '$to'),
      op_param('$top', 'query', '$top', '$top'),
      op_param('@odata.context', 'body', '@odata.context', '@odata.context'),
      op_param('@odata.count', 'body', '@odata.count', '@odata.count'),
      op_param('api-version', 'query', 'api_version', 'api_version'),
      op_param('authorizationNamespace', 'path', 'authorization_namespace', 'authorization_namespace'),
      op_param('policyAssignmentName', 'path', 'policy_assignment_name', 'policy_assignment_name'),
      op_param('policyStatesResource', 'path', 'policy_states_resource', 'policy_states_resource'),
...
def task
  ...
  params = STDIN.read
  result = policy_states_list_query_results_for_resource_group_level_policy_assignment(params)
