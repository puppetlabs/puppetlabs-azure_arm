...
def workspace_collections_delete(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.PowerBI/workspaceCollections/%{workspace_collection_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('workspaceCollectionName', 'path', 'name', 'workspace_collection_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = WorkspaceCollections_delete(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = workspace_collections_delete(params)
