...
def policy_assignments_create_by_id(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('policyAssignmentId', 'path', 'policy_assignment_id', 'policy_assignment_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = policy_assignments_create_by_id(params)
