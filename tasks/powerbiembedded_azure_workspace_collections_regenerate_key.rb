...
def workspace_collections_regenerate_key(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.PowerBI/workspaceCollections/%{workspace_collection_name}/regenerateKey" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('key1', 'body', 'key1', 'key1'),
      op_param('key2', 'body', 'key2', 'key2'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('workspaceCollectionName', 'path', 'workspace_collection_name', 'workspace_collection_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = WorkspaceCollections_regenerateKey(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = workspace_collections_regenerate_key(params)
