...
def detect_image_url_with_no_store(*args)
  header_params = {}
  ...
  uri_string = "https://southcentralus.api.cognitive.microsoft.com//%{project_id}/detect/iterations/%{published_name}/url/nostore" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('publishedName', 'path', 'published_name', 'published_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = DetectImageUrlWithNoStore(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = detect_image_url_with_no_store(params)
