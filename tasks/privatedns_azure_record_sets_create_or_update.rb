...
def record_sets_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/privateDnsZones/%{private_zone_name}/%{record_type}/%{relative_record_set_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('privateZoneName', 'path', 'private_zone_name', 'private_zone_name'),
      op_param('recordType', 'path', 'record_type', 'record_type'),
      op_param('relativeRecordSetName', 'path', 'relative_record_set_name', 'relative_record_set_name'),
...
def task
  ...
  params = STDIN.read
  result = record_sets_create_or_update(params)
