...
def virtual_network_links_delete(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/privateDnsZones/%{private_zone_name}/virtualNetworkLinks/%{virtual_network_link_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('virtualNetworkLinkName', 'path', 'virtual_network_link_name', 'virtual_network_link_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_network_links_delete(params)
