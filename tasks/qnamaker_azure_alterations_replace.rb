...
def alterations_replace(*args)
  ...
  uri_string = "https:////alterations" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('wordAlterations', 'body', 'word_alterations', 'word_alterations'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = alterations_replace(params)
