...
def knowledgebase_replace(*args)
  header_params = {}
  ...
  uri_string = "https:////knowledgebases/%{kb_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('replaceKb', 'body', 'replace_kb', 'replace_kb'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = knowledgebase_replace(params)
