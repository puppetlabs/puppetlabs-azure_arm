...
def runtime_generate_answer(*args)
  header_params = {}
  ...
  uri_string = "https:////knowledgebases/%{kb_id}/generateAnswer" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('answers', 'body', 'answers', 'answers'),
      op_param('generateAnswerPayload', 'body', 'generate_answer_payload', 'generate_answer_payload'),
      op_param('kbId', 'path', 'kb_id', 'kb_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate(header_params)
  header_params["Authorization"] = ENV["<no value>_api_key"]
end
...
def task
  ...
  params = STDIN.read
  result = runtime_generate_answer(params)
