...
def protected_items_delete(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//Subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.RecoveryServices/vaults/%{vault_name}/backupFabrics/%{fabric_name}/protectionContainers/%{container_name}/protectedItems/%{protected_item_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('backupmanagementtype', 'body', 'backup_management_type', 'backupmanagementtype'),
      op_param('containerName', 'path', 'container_name', 'container_name'),
      op_param('fabricName', 'path', 'fabric_name', 'fabric_name'),
      op_param('lastrecoverypoint', 'body', 'last_recovery_point', 'lastrecoverypoint'),
      op_param('policyid', 'body', 'policy_id', 'policyid'),
      op_param('protectedItemName', 'path', 'protected_item_name', 'protected_item_name'),
      op_param('protecteditemtype', 'body', 'protected_item_type', 'protecteditemtype'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('sourceresourceid', 'body', 'source_resource_id', 'sourceresourceid'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('vaultName', 'path', 'vault_name', 'vault_name'),
      op_param('workloadtype', 'body', 'workload_type', 'workloadtype'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = protected_items_delete(params)
