...
def redis_force_reboot(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Cache/Redis/%{name}/forceReboot" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('reboottype', 'body', 'reboot_type', 'reboottype'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('shardid', 'body', 'shard_id', 'shardid'),
...
def task
  ...
  params = STDIN.read
  result = redis_force_reboot(params)
