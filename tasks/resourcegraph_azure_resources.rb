...
def resources(*args)
  ...
  uri_string = "https://management.azure.com//providers/Microsoft.ResourceGraph/resources" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('facets', 'body', 'facets', 'facets'),
      op_param('query', 'body', 'query', 'query'),
      op_param('resulttruncated', 'body', 'result_truncated', 'resulttruncated'),
      op_param('totalrecords', 'body', 'total_records', 'totalrecords'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
