...
def create_service_from_template(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/GetServices/$/CreateFromTemplate" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ServiceFromTemplateDescription', 'body', 'service_from_template_description', 'service_from_template_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = CreateServiceFromTemplate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = create_service_from_template(params)
