...
def get_application_type_info_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ApplicationTypes" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('applicationtypedefinitionkind', 'body', 'application_type_definition_kind', 'applicationtypedefinitionkind'),
      op_param('ApplicationTypeDefinitionKindFilter', 'query', 'application_type_definition_kind_filter', 'application_type_definition_kind_filter'),
      op_param('ContinuationToken', 'query', 'continuation_token', 'continuation_token'),
      op_param('defaultparameterlist', 'body', 'default_parameter_list', 'defaultparameterlist'),
      ...
      op_param('statusdetails', 'body', 'status_details', 'statusdetails'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetApplicationTypeInfoList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_application_type_info_list(params)
