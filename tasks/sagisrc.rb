...
def get_image_store_root_content(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ImageStore" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('filesize', 'body', 'file_size', 'filesize'),
      op_param('fileversion', 'body', 'file_version', 'fileversion'),
      op_param('modifieddate', 'body', 'modified_date', 'modifieddate'),
      op_param('storerelativepath', 'body', 'store_relative_path', 'storerelativepath'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetImageStoreRootContent(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_image_store_root_content(params)
