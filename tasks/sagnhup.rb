...
def get_node_health_using_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Nodes/%{node_name}/$/GetHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ClusterHealthPolicy', 'body', 'cluster_health_policy', 'cluster_health_policy'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetNodeHealthUsingPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_node_health_using_policy(params)
