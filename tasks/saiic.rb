...
def invoke_infrastructure_command(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/InvokeInfrastructureCommand" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('Command', 'query', 'command', 'command'),
      op_param('ServiceId', 'query', 'service_id', 'service_id'),
...
def authenticate
  ...
  params = STDIN.read
  result = InvokeInfrastructureCommand(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = invoke_infrastructure_command(params)
