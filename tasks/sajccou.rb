...
def job_collections_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Scheduler/jobCollections/%{job_collection_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('jobCollection', 'body', 'job_collection', 'job_collection'),
      op_param('jobCollectionName', 'path', 'job_collection_name', 'job_collection_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = JobCollections_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = job_collections_create_or_update(params)
