...
def replication_migration_items_test_migrate(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//Subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.RecoveryServices/vaults/%{resource_name}/replicationFabrics/%{fabric_name}/replicationProtectionContainers/%{protection_container_name}/replicationMigrationItems/%{migration_item_name}/testMigrate" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('testMigrateInput', 'body', 'test_migrate_input', 'test_migrate_input'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = replication_migration_items_test_migrate(params)
