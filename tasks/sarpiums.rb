...
def replication_protected_items_update_mobility_service(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('replicationProtectedItemName', 'path', 'replication_protected_item_name', 'replication_protected_item_name'),
      ...
      op_param('updateMobilityServiceRequest', 'body', 'update_mobility_service_request', 'update_mobility_service_request'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = replication_protected_items_update_mobility_service(params)
