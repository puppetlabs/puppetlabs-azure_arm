...
def set_upgrade_orchestration_service_state(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/SetUpgradeOrchestrationServiceState" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('UpgradeOrchestrationServiceState', 'body', 'upgrade_orchestration_service_state', 'upgrade_orchestration_service_state'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = SetUpgradeOrchestrationServiceState(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = set_upgrade_orchestration_service_state(params)
