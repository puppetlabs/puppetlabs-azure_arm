...
def update_repair_execution_state(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/UpdateRepairExecutionState" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RepairTask', 'body', 'repair_task', 'repair_task'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = UpdateRepairExecutionState(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = update_repair_execution_state(params)
