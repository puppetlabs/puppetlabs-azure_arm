...
def documents_autocomplete_get(*args)
  header_params = {}
  ...
  uri_string = "https:////docs/search.autocomplete" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('autocompleteMode', 'query', 'autocomplete_mode', 'autocomplete_mode'),
      op_param('client-request-id', 'header', 'client_request_id', 'client_request_id'),
      op_param('fuzzy', 'query', 'fuzzy', 'fuzzy'),
      ...
      op_param('suggesterName', 'query', 'suggester_name', 'suggester_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = Documents_AutocompleteGet(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = documents_autocomplete_get(params)
