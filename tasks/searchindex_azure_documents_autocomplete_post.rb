...
def documents_autocomplete_post(*args)
  header_params = {}
  ...
  uri_string = "https:////docs/search.post.autocomplete" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('autocompletemode', 'body', 'autocomplete_mode', 'autocompletemode'),
      op_param('autocompleteRequest', 'body', 'autocomplete_request', 'autocomplete_request'),
      op_param('client-request-id', 'header', 'client_request_id', 'client_request_id'),
      op_param('filter', 'body', 'filter', 'filter'),
      op_param('fuzzy', 'body', 'fuzzy', 'fuzzy'),
      op_param('highlightposttag', 'body', 'highlight_post_tag', 'highlightposttag'),
      op_param('highlightpretag', 'body', 'highlight_pre_tag', 'highlightpretag'),
      op_param('minimumcoverage', 'body', 'minimum_coverage', 'minimumcoverage'),
      op_param('search', 'body', 'search', 'search'),
      op_param('searchfields', 'body', 'search_fields', 'searchfields'),
      op_param('suggestername', 'body', 'suggester_name', 'suggestername'),
      op_param('top', 'body', 'top', 'top'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Documents_AutocompletePost(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = documents_autocomplete_post(params)
