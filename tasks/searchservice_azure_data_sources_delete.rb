...
def data_sources_delete(*args)
  ...
  uri_string = "https:////datasources('{dataSourceName}')" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('container', 'body', 'container', 'container'),
      op_param('credentials', 'body', 'credentials', 'credentials'),
      op_param('datachangedetectionpolicy', 'body', 'data_change_detection_policy', 'datachangedetectionpolicy'),
      op_param('datadeletiondetectionpolicy', 'body', 'data_deletion_detection_policy', 'datadeletiondetectionpolicy'),
      op_param('dataSourceName', 'path', 'name', 'data_source_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = DataSources_Delete(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = data_sources_delete(params)
