...
def get_service_statistics(*args)
  header_params = {}
  ...
  uri_string = "https:////servicestats" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('counters', 'body', 'counters', 'counters'),
      op_param('limits', 'body', 'limits', 'limits'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = GetServiceStatistics(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_service_statistics(params)
