...
def indexers_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https:////indexers('{indexerName}')" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('datasourcename', 'body', 'data_source_name', 'datasourcename'),
      op_param('description', 'body', 'description', 'description'),
      op_param('disabled', 'body', 'disabled', 'disabled'),
      op_param('fieldmappings', 'body', 'field_mappings', 'fieldmappings'),
      op_param('If-Match', 'header', 'if_match', 'if_match'),
      op_param('If-None-Match', 'header', 'if_none_match', 'if_none_match'),
      op_param('indexer', 'body', 'indexer', 'indexer'),
      op_param('indexerName', 'path', 'name', 'indexer_name'),
      op_param('name', 'body', 'name', 'name'),
      op_param('outputfieldmappings', 'body', 'output_field_mappings', 'outputfieldmappings'),
      ...
      op_param('skillsetname', 'body', 'skillset_name', 'skillsetname'),
      op_param('targetindexname', 'body', 'target_index_name', 'targetindexname'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Indexers_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = indexers_create_or_update(params)
