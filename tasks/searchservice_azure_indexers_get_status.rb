...
def indexers_get_status(*args)
  ...
  uri_string = "https:////indexers('{indexerName}')/search.status" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('datasourcename', 'body', 'datasourcename', 'datasourcename'),
      op_param('description', 'body', 'description', 'description'),
      op_param('disabled', 'body', 'disabled', 'disabled'),
      op_param('fieldmappings', 'body', 'fieldmappings', 'fieldmappings'),
      op_param('indexerName', 'path', 'indexer_name', 'indexer_name'),
      op_param('name', 'body', 'name', 'name'),
      op_param('outputfieldmappings', 'body', 'outputfieldmappings', 'outputfieldmappings'),
      op_param('parameters', 'body', 'parameters', 'parameters'),
      op_param('schedule', 'body', 'schedule', 'schedule'),
      op_param('skillsetname', 'body', 'skillsetname', 'skillsetname'),
      op_param('targetindexname', 'body', 'targetindexname', 'targetindexname'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Indexers_GetStatus(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = indexers_get_status(params)
