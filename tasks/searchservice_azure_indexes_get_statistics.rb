...
def indexes_get_statistics(*args)
  header_params = {}
  ...
  uri_string = "https:////indexes('{indexName}')/search.stats" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('documentcount', 'body', 'document_count', 'documentcount'),
      op_param('indexName', 'path', 'index_name', 'index_name'),
      op_param('storagesize', 'body', 'storage_size', 'storagesize'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Indexes_GetStatistics(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = indexes_get_statistics(params)
