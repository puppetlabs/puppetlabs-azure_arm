...
def skillsets_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https:////skillsets('{skillsetName}')" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('cognitiveservices', 'body', 'cognitive_services', 'cognitiveservices'),
      op_param('description', 'body', 'description', 'description'),
      op_param('name', 'body', 'name', 'name'),
      op_param('Prefer', 'header', 'prefer', 'prefer'),
      op_param('skills', 'body', 'skills', 'skills'),
      op_param('skillset', 'body', 'skillset', 'skillset'),
      op_param('skillsetName', 'path', 'name', 'skillset_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = Skillsets_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = skillsets_create_or_update(params)
