...
def synonym_maps_delete(*args)
  ...
  uri_string = "https:////synonymmaps('{synonymMapName}')" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('synonymMapName', 'path', 'name', 'synonym_map_name'),
      op_param('synonyms', 'body', 'synonyms', 'synonyms'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = SynonymMaps_Delete(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = synonym_maps_delete(params)
