...
def replication_migration_items_test_migrate_cleanup(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//Subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.RecoveryServices/vaults/%{resource_name}/replicationFabrics/%{fabric_name}/replicationProtectionContainers/%{protection_container_name}/replicationMigrationItems/%{migration_item_name}/testMigrateCleanup" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('migrationItemName', 'path', 'migration_item_name', 'migration_item_name'),
      op_param('properties', 'body', 'properties', 'properties'),
      op_param('protectionContainerName', 'path', 'protection_container_name', 'protection_container_name'),
      ...
      op_param('testMigrateCleanupInput', 'body', 'test_migrate_cleanup_input', 'test_migrate_cleanup_input'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = replication_migration_items_test_migrate_cleanup(params)
