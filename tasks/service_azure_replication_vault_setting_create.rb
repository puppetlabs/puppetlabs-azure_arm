...
def replication_vault_setting_create(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//Subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.RecoveryServices/vaults/%{resource_name}/replicationVaultSettings/%{vault_setting_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('vaultSettingName', 'path', 'name', 'vault_setting_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = replication_vault_setting_create(params)
