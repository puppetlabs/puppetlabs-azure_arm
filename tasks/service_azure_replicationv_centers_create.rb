...
def replicationv_centers_create(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      op_param('addVCenterRequest', 'body', 'add_v_center_request', 'add_v_center_request'),
      ...
      op_param('vCenterName', 'path', 'v_center_name', 'v_center_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = replicationv_centers_create(params)
