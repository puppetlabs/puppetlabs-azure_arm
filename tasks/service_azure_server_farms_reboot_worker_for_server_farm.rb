...
def server_farms_reboot_worker_for_server_farm(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/serverfarms/%{name}/workers/%{worker_name}/reboot" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('workerName', 'path', 'worker_name', 'worker_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = server_farms_reboot_worker_for_server_farm(params)
