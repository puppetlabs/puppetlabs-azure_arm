...
def sites_create_or_update_site_vnet_connection_gateway_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/virtualNetworkConnections/%{vnet_name}/gateways/%{gateway_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('connectionEnvelope', 'body', 'connection_envelope', 'connection_envelope'),
      ...
      op_param('vnetName', 'path', 'vnet_name', 'vnet_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = sites_create_or_update_site_vnet_connection_gateway_slot(params)
