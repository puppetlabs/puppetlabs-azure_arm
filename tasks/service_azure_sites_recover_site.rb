...
def sites_recover_site(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/recover" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('recoverconfig', 'body', 'recoverconfig', 'recoverconfig'),
      ...
      op_param('sitename', 'body', 'sitename', 'sitename'),
      op_param('slotname', 'body', 'slotname', 'slotname'),
      op_param('snapshottime', 'body', 'snapshottime', 'snapshottime'),
...
def task
  ...
  params = STDIN.read
  result = sites_recover_site(params)
