...
def sites_recover_site_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/recover" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('recoverconfig', 'body', 'recover_config', 'recoverconfig'),
      op_param('recoveryEntity', 'body', 'recovery_entity', 'recovery_entity'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('sitename', 'body', 'site_name', 'sitename'),
      op_param('slot', 'path', 'slot', 'slot'),
      op_param('slotname', 'body', 'slot_name', 'slotname'),
      op_param('snapshottime', 'body', 'snapshot_time', 'snapshottime'),
...
def task
  ...
  params = STDIN.read
  result = sites_recover_site_slot(params)
