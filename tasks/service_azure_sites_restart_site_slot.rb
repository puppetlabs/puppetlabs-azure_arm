...
def sites_restart_site_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/restart" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('softRestart', 'query', 'soft_restart', 'soft_restart'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('synchronous', 'query', 'synchronous', 'synchronous'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = sites_restart_site_slot(params)
