...
def namespaces_migrate(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.ServiceBus/namespaces/%{namespace_name}/migrate" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('targetnamespacetype', 'body', 'target_namespace_type', 'targetnamespacetype'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = namespaces_migrate(params)
