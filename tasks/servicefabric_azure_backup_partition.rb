...
def backup_partition(*args)
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/Backup" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('BackupPartitionDescription', 'body', 'backup_partition_description', 'backup_partition_description'),
      op_param('BackupTimeout', 'query', 'backup_timeout', 'backup_timeout'),
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = backup_partition(params)
