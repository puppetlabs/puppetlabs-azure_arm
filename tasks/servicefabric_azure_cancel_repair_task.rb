...
def cancel_repair_task(*args)
  ...
  uri_string = "https://localhost:19080//$/CancelRepairTask" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RepairTaskCancelDescription', 'body', 'repair_task_cancel_description', 'repair_task_cancel_description'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = CancelRepairTask(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = cancel_repair_task(params)
