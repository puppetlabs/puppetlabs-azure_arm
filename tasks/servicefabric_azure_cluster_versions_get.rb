...
def cluster_versions_get(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.ServiceFabric/locations/%{location}/environments/%{environment}/clusterVersions/%{cluster_version}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('clusterVersion', 'path', 'cluster_version', 'cluster_version'),
      op_param('environment', 'path', 'environment', 'environment'),
...
def task
  ...
  params = STDIN.read
  result = cluster_versions_get(params)
