...
def create_compose_deployment(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ComposeDeployments/$/Create" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('CreateComposeDeploymentDescription', 'body', 'create_compose_deployment_description', 'create_compose_deployment_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = CreateComposeDeployment(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = create_compose_deployment(params)
