...
def delete_repair_task(*args)
  ...
  uri_string = "https://localhost:19080//$/DeleteRepairTask" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RepairTaskDeleteDescription', 'body', 'repair_task_delete_description', 'repair_task_delete_description'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = DeleteRepairTask(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = delete_repair_task(params)
