...
def disable_application_backup(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/DisableBackup" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('DisableBackupDescription', 'body', 'disable_backup_description', 'disable_backup_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = DisableApplicationBackup(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = disable_application_backup(params)
