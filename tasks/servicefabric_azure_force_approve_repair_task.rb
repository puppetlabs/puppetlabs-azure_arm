...
def force_approve_repair_task(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/ForceApproveRepairTask" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RepairTaskApproveDescription', 'body', 'repair_task_approve_description', 'repair_task_approve_description'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = ForceApproveRepairTask(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = force_approve_repair_task(params)
