...
def get_application_backup_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/GetBackups" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('EndDateTimeFilter', 'query', 'end_date_time_filter', 'end_date_time_filter'),
      op_param('items', 'body', 'items', 'items'),
      op_param('Latest', 'query', 'latest', 'latest'),
      op_param('MaxResults', 'query', 'max_results', 'max_results'),
      op_param('StartDateTimeFilter', 'query', 'start_date_time_filter', 'start_date_time_filter'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetApplicationBackupList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_application_backup_list(params)
