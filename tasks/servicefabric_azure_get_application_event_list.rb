...
def get_application_event_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//EventsStore/Applications/%{application_id}/$/Events" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('EndTimeUtc', 'query', 'end_time_utc', 'end_time_utc'),
      op_param('error', 'body', 'error', 'error'),
      op_param('EventsTypesFilter', 'query', 'events_types_filter', 'events_types_filter'),
      op_param('ExcludeAnalysisEvents', 'query', 'exclude_analysis_events', 'exclude_analysis_events'),
      op_param('SkipCorrelationLookup', 'query', 'skip_correlation_lookup', 'skip_correlation_lookup'),
      op_param('StartTimeUtc', 'query', 'start_time_utc', 'start_time_utc'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetApplicationEventList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_application_event_list(params)
