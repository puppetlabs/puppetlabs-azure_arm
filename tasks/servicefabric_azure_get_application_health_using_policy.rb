...
def get_application_health_using_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/GetHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ApplicationHealthPolicy', 'body', 'application_health_policy', 'application_health_policy'),
      op_param('applicationId', 'path', 'application_id', 'application_id'),
      op_param('deployedapplicationhealthstates', 'body', 'deployed_application_health_states', 'deployedapplicationhealthstates'),
      op_param('DeployedApplicationsHealthStateFilter', 'query', 'deployed_applications_health_state_filter', 'deployed_applications_health_state_filter'),
      ...
      op_param('healthevents', 'body', 'health_events', 'healthevents'),
      op_param('healthstatistics', 'body', 'health_statistics', 'healthstatistics'),
      op_param('name', 'body', 'name', 'name'),
      op_param('servicehealthstates', 'body', 'service_health_states', 'servicehealthstates'),
      op_param('ServicesHealthStateFilter', 'query', 'services_health_state_filter', 'services_health_state_filter'),
      op_param('timeout', 'query', 'timeout', 'timeout'),
      op_param('unhealthyevaluations', 'body', 'unhealthy_evaluations', 'unhealthyevaluations'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = GetApplicationHealthUsingPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_application_health_using_policy(params)
