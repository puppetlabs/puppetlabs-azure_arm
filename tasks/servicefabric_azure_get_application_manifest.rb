...
def get_application_manifest(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ApplicationTypes/%{application_type_name}/$/GetApplicationManifest" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ApplicationTypeVersion', 'query', 'application_type_version', 'application_type_version'),
      op_param('manifest', 'body', 'manifest', 'manifest'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetApplicationManifest(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_application_manifest(params)
