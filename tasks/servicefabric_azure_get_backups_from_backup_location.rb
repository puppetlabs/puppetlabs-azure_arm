...
def get_backups_from_backup_location(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//BackupRestore/$/GetBackups" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('GetBackupByStorageQueryDescription', 'body', 'get_backup_by_storage_query_description', 'get_backup_by_storage_query_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetBackupsFromBackupLocation(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_backups_from_backup_location(params)
