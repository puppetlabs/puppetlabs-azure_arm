...
def get_cluster_configuration(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/GetClusterConfiguration" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('api-version', 'query', 'cluster_configuration', 'api_version'),
      op_param('clusterconfiguration', 'body', 'cluster_configuration', 'clusterconfiguration'),
      op_param('ConfigurationApiVersion', 'query', 'cluster_configuration', 'configuration_api_version'),
      op_param('timeout', 'query', 'cluster_configuration', 'timeout'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = GetClusterConfiguration(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_cluster_configuration(params)
