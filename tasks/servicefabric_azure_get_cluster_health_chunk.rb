...
def get_cluster_health_chunk(*args)
  header_params = {}
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('applicationhealthstatechunks', 'body', 'application_health_state_chunks', 'applicationhealthstatechunks'),
      op_param('healthstate', 'body', 'health_state', 'healthstate'),
      op_param('nodehealthstatechunks', 'body', 'node_health_state_chunks', 'nodehealthstatechunks'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetClusterHealthChunk(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_cluster_health_chunk(params)
