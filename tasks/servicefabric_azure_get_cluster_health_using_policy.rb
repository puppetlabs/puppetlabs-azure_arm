...
def get_cluster_health_using_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/GetClusterHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ClusterHealthPolicies', 'body', 'cluster_health_policies', 'cluster_health_policies'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetClusterHealthUsingPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_cluster_health_using_policy(params)
