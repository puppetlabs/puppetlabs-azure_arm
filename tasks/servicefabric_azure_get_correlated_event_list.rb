...
def get_correlated_event_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//EventsStore/CorrelatedEvents/%{event_instance_id}/$/Events" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('eventInstanceId', 'path', 'event_instance_id', 'event_instance_id'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetCorrelatedEventList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_correlated_event_list(params)
