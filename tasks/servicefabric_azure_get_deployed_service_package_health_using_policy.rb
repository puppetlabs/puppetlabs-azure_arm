...
def get_deployed_service_package_health_using_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Nodes/%{node_name}/$/GetApplications/%{application_id}/$/GetServicePackages/%{service_package_name}/$/GetHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('servicePackageName', 'path', 'service_package_name', 'service_package_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetDeployedServicePackageHealthUsingPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_deployed_service_package_health_using_policy(params)
