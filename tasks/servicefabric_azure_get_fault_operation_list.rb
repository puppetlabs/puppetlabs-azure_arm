...
def get_fault_operation_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Faults/" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('TypeFilter', 'query', 'type_filter', 'type_filter'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = GetFaultOperationList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_fault_operation_list(params)
