...
def get_image_store_content(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ImageStore/%{content_path}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('storefiles', 'body', 'storefiles', 'storefiles'),
      op_param('storefolders', 'body', 'storefolders', 'storefolders'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetImageStoreContent(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_image_store_content(params)
