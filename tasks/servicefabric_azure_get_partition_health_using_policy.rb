...
def get_partition_health_using_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/GetHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ReplicasHealthStateFilter', 'query', 'replicas_health_state_filter', 'replicas_health_state_filter'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetPartitionHealthUsingPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_partition_health_using_policy(params)
