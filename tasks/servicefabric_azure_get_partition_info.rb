...
def get_partition_info(*args)
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('partitionstatus', 'body', 'partition_status', 'partitionstatus'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetPartitionInfo(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_partition_info(params)
