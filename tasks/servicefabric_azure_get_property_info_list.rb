...
def get_property_info_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Names/%{name_id}/$/GetProperties" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('IncludeValues', 'query', 'include_values', 'include_values'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetPropertyInfoList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_property_info_list(params)
