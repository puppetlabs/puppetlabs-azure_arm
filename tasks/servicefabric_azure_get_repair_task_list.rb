...
def get_repair_task_list(*args)
  ...
  uri_string = "https://localhost:19080//$/GetRepairTaskList" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ExecutorFilter', 'query', 'executor_filter', 'executor_filter'),
      op_param('StateFilter', 'query', 'state_filter', 'state_filter'),
      op_param('TaskIdFilter', 'query', 'task_id_filter', 'task_id_filter'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = GetRepairTaskList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_repair_task_list(params)
