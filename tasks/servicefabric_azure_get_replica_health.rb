...
def get_replica_health(*args)
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/GetReplicas/%{replica_id}/$/GetHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('partition_id', 'path', 'partition_id', 'partition_id'),
      op_param('partitionid', 'body', 'partition_id', 'partitionid'),
      op_param('replicaId', 'path', 'replica_id', 'replica_id'),
      op_param('servicekind', 'body', 'service_kind', 'servicekind'),
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_replica_health(params)
