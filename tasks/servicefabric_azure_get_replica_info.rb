...
def get_replica_info(*args)
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/GetReplicas/%{replica_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      op_param('address', 'body', 'address', 'address'),
      op_param('api-version', 'query', 'api_version', 'api_version'),
      op_param('healthstate', 'body', 'health_state', 'healthstate'),
      op_param('lastinbuilddurationinseconds', 'body', 'last_in_build_duration_in_seconds', 'lastinbuilddurationinseconds'),
      ...
      op_param('replicastatus', 'body', 'replica_status', 'replicastatus'),
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_replica_info(params)
