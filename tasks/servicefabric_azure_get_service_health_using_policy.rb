...
def get_service_health_using_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Services/%{service_id}/$/GetHealth" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('PartitionsHealthStateFilter', 'query', 'partitions_health_state_filter', 'partitions_health_state_filter'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetServiceHealthUsingPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_service_health_using_policy(params)
