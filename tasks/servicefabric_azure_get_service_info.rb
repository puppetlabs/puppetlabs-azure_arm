...
def get_service_info(*args)
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/GetServices/%{service_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('isservicegroup', 'body', 'is_service_group', 'isservicegroup'),
      op_param('manifestversion', 'body', 'manifest_version', 'manifestversion'),
      op_param('name', 'body', 'name', 'name'),
      op_param('serviceId', 'path', 'service_id', 'service_id'),
      op_param('servicekind', 'body', 'service_kind', 'servicekind'),
      op_param('servicestatus', 'body', 'service_status', 'servicestatus'),
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_service_info(params)
