...
def get_service_info_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/GetServices" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ServiceTypeName', 'query', 'service_type_name', 'service_type_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetServiceInfoList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_service_info_list(params)
