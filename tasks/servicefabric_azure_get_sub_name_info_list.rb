...
def get_sub_name_info_list(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Names/%{name_id}/$/GetSubNames" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('isconsistent', 'body', 'is_consistent', 'isconsistent'),
      op_param('nameId', 'path', 'name_id', 'name_id'),
      op_param('Recursive', 'query', 'recursive', 'recursive'),
      op_param('subnames', 'body', 'sub_names', 'subnames'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetSubNameInfoList(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_sub_name_info_list(params)
