...
def mesh_gateway_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Resources/Gateways/%{gateway_resource_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('gatewayResourceDescription', 'body', 'gateway_resource_description', 'gateway_resource_description'),
      op_param('gatewayResourceName', 'path', 'gateway_resource_name', 'gateway_resource_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = MeshGateway_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = mesh_gateway_create_or_update(params)
