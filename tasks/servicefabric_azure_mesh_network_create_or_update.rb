...
def mesh_network_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Resources/Networks/%{network_resource_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('networkResourceDescription', 'body', 'network_resource_description', 'network_resource_description'),
      op_param('networkResourceName', 'path', 'network_resource_name', 'network_resource_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = MeshNetwork_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = mesh_network_create_or_update(params)
