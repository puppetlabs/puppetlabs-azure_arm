...
def mesh_secret_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Resources/Secrets/%{secret_resource_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('secretResourceDescription', 'body', 'secret_resource_description', 'secret_resource_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = MeshSecret_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = mesh_secret_create_or_update(params)
