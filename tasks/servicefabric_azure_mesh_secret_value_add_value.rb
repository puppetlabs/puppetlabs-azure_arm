...
def mesh_secret_value_add_value(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Resources/Secrets/%{secret_resource_name}/values/%{secret_value_resource_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('secretValueResourceDescription', 'body', 'secret_value_resource_description', 'secret_value_resource_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = MeshSecretValue_AddValue(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = mesh_secret_value_add_value(params)
