...
def mesh_service_replica_get(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Resources/Applications/%{application_resource_name}/Services/%{service_resource_name}/Replicas/%{replica_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('applicationResourceName', 'path', 'application_resource_name', 'application_resource_name'),
      op_param('codepackages', 'body', 'code_packages', 'codepackages'),
      op_param('diagnostics', 'body', 'diagnostics', 'diagnostics'),
      op_param('networkrefs', 'body', 'network_refs', 'networkrefs'),
      op_param('ostype', 'body', 'os_type', 'ostype'),
      op_param('replica_name', 'path', 'replica_name', 'replica_name'),
      op_param('replicaname', 'body', 'replica_name', 'replicaname'),
      op_param('serviceResourceName', 'path', 'service_resource_name', 'service_resource_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = MeshServiceReplica_Get(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = mesh_service_replica_get(params)
