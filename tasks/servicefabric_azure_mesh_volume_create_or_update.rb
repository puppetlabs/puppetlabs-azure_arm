...
def mesh_volume_create_or_update(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Resources/Volumes/%{volume_resource_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('volumeResourceDescription', 'body', 'volume_resource_description', 'volume_resource_description'),
      op_param('volumeResourceName', 'path', 'volume_resource_name', 'volume_resource_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = MeshVolume_CreateOrUpdate(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = mesh_volume_create_or_update(params)
