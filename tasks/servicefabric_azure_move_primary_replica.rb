...
def move_primary_replica(*args)
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/MovePrimaryReplica" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('NodeName', 'query', 'node_name', 'node_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = MovePrimaryReplica(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = move_primary_replica(params)
