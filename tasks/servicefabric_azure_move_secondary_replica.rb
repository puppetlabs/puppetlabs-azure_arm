...
def move_secondary_replica(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/MoveSecondaryReplica" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('CurrentNodeName', 'query', 'current_node_name', 'current_node_name'),
      op_param('error', 'body', 'error', 'error'),
      op_param('IgnoreConstraints', 'query', 'ignore_constraints', 'ignore_constraints'),
      op_param('NewNodeName', 'query', 'new_node_name', 'new_node_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = MoveSecondaryReplica(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = move_secondary_replica(params)
