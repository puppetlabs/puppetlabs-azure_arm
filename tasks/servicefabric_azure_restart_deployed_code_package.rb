...
def restart_deployed_code_package(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Nodes/%{node_name}/$/GetApplications/%{application_id}/$/GetCodePackages/$/Restart" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RestartDeployedCodePackageDescription', 'body', 'restart_deployed_code_package_description', 'restart_deployed_code_package_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = RestartDeployedCodePackage(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = restart_deployed_code_package(params)
