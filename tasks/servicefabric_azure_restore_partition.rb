...
def restore_partition(*args)
  ...
  uri_string = "https://localhost:19080//Partitions/%{partition_id}/$/Restore" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RestorePartitionDescription', 'body', 'restore_partition_description', 'restore_partition_description'),
      op_param('RestoreTimeout', 'query', 'restore_timeout', 'restore_timeout'),
...
def authenticate
  ...
  params = STDIN.read
  result = RestorePartition(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = restore_partition(params)
