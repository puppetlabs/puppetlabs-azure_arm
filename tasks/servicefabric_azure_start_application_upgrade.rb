...
def start_application_upgrade(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Applications/%{application_id}/$/Upgrade" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ApplicationUpgradeDescription', 'body', 'application_upgrade_description', 'application_upgrade_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = StartApplicationUpgrade(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = start_application_upgrade(params)
