...
def start_cluster_upgrade(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/Upgrade" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('StartClusterUpgradeDescription', 'body', 'start_cluster_upgrade_description', 'start_cluster_upgrade_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = StartClusterUpgrade(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = start_cluster_upgrade(params)
