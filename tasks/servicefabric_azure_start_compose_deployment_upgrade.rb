...
def start_compose_deployment_upgrade(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ComposeDeployments/%{deployment_name}/$/Upgrade" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('ComposeDeploymentUpgradeDescription', 'body', 'compose_deployment_upgrade_description', 'compose_deployment_upgrade_description'),
...
def authenticate
  ...
  params = STDIN.read
  result = StartComposeDeploymentUpgrade(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = start_compose_deployment_upgrade(params)
