...
def start_node_transition(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Faults/Nodes/%{node_name}/$/StartTransition/" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('NodeInstanceId', 'query', 'node_instance_id', 'node_instance_id'),
      op_param('nodeName', 'path', 'node_name', 'node_name'),
      op_param('NodeTransitionType', 'query', 'node_transition_type', 'node_transition_type'),
      op_param('OperationId', 'query', 'operation_id', 'operation_id'),
      op_param('StopDurationInSeconds', 'query', 'stop_duration_in_seconds', 'stop_duration_in_seconds'),
...
def authenticate
  ...
  params = STDIN.read
  result = StartNodeTransition(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = start_node_transition(params)
