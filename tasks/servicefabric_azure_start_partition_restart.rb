...
def start_partition_restart(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Faults/Services/%{service_id}/$/GetPartitions/%{partition_id}/$/StartRestart" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RestartPartitionMode', 'query', 'restart_partition_mode', 'restart_partition_mode'),
...
def authenticate
  ...
  params = STDIN.read
  result = StartPartitionRestart(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = start_partition_restart(params)
