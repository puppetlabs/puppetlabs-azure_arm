...
def start_quorum_loss(*args)
  ...
  uri_string = "https://localhost:19080//Faults/Services/%{service_id}/$/GetPartitions/%{partition_id}/$/StartQuorumLoss" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('QuorumLossDuration', 'query', 'quorum_loss_duration', 'quorum_loss_duration'),
      op_param('QuorumLossMode', 'query', 'quorum_loss_mode', 'quorum_loss_mode'),
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = start_quorum_loss(params)
