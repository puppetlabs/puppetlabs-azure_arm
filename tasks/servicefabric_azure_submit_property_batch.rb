...
def submit_property_batch(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//Names/%{name_id}/$/GetProperties/$/SubmitBatch" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('PropertyBatchDescriptionList', 'body', 'property_batch_description_list', 'property_batch_description_list'),
...
def authenticate
  ...
  params = STDIN.read
  result = SubmitPropertyBatch(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = submit_property_batch(params)
