...
def unprovision_application_type(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//ApplicationTypes/%{application_type_name}/$/Unprovision" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('applicationTypeName', 'path', 'application_type_name', 'application_type_name'),
      op_param('error', 'body', 'error', 'error'),
      op_param('timeout', 'query', 'timeout', 'timeout'),
      op_param('UnprovisionApplicationTypeDescriptionInfo', 'body', 'unprovision_application_type_description_info', 'unprovision_application_type_description_info'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = UnprovisionApplicationType(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = unprovision_application_type(params)
