...
def unprovision_cluster(*args)
  ...
  uri_string = "https://localhost:19080//$/Unprovision" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('UnprovisionFabricDescription', 'body', 'unprovision_fabric_description', 'unprovision_fabric_description'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = UnprovisionCluster(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = unprovision_cluster(params)
