...
def update_backup_policy(*args)
  ...
  uri_string = "https://localhost:19080//BackupRestore/BackupPolicies/%{backup_policy_name}/$/Update" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('BackupPolicyDescription', 'body', 'backup_policy_description', 'backup_policy_description'),
      op_param('backupPolicyName', 'path', 'backup_policy_name', 'backup_policy_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = UpdateBackupPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = update_backup_policy(params)
