...
def update_cluster_upgrade(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/UpdateUpgrade" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('UpdateClusterUpgradeDescription', 'body', 'update_cluster_upgrade_description', 'update_cluster_upgrade_description'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = UpdateClusterUpgrade(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = update_cluster_upgrade(params)
