...
def update_repair_task_health_policy(*args)
  header_params = {}
  ...
  uri_string = "https://localhost:19080//$/UpdateRepairTaskHealthPolicy" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('RepairTaskUpdateHealthPolicyDescription', 'body', 'repair_task_update_health_policy_description', 'repair_task_update_health_policy_description'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def authenticate
  ...
  params = STDIN.read
  result = UpdateRepairTaskHealthPolicy(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = update_repair_task_health_policy(params)
