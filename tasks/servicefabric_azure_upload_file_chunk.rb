...
def upload_file_chunk(*args)
  ...
  uri_string = "https://localhost:19080//ImageStore/%{content_path}/$/UploadChunk" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('Content-Range', 'header', 'content_range', 'content_range'),
      op_param('contentPath', 'path', 'content_path', 'content_path'),
      op_param('error', 'body', 'error', 'error'),
      op_param('session-id', 'query', 'session_id', 'session_id'),
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = upload_file_chunk(params)
