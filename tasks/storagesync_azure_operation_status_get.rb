...
def operation_status_get(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.StorageSync/locations/%{location_name}/workflows/%{workflow_id}/operations/%{operation_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('locationName', 'path', 'location_name', 'location_name'),
      op_param('operationId', 'path', 'operation_id', 'operation_id'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('workflowId', 'path', 'workflow_id', 'workflow_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = operation_status_get(params)
