...
def backups_clone(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.StorSimple/managers/%{manager_name}/devices/%{device_name}/backups/%{backup_name}/elements/%{backup_element_name}/clone" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('backupelement', 'body', 'backup_element', 'backupelement'),
      op_param('backupElementName', 'path', 'backup_element_name', 'backup_element_name'),
      op_param('backupName', 'path', 'backup_name', 'backup_name'),
      ...
      op_param('targetaccesscontrolrecordids', 'body', 'target_access_control_record_ids', 'targetaccesscontrolrecordids'),
      op_param('targetdeviceid', 'body', 'target_device_id', 'targetdeviceid'),
      op_param('targetvolumename', 'body', 'target_volume_name', 'targetvolumename'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
