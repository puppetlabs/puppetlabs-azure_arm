...
def devices_failover(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.StorSimple/managers/%{manager_name}/devices/%{source_device_name}/failover" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('sourceDeviceName', 'path', 'source_device_name', 'source_device_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('targetdeviceid', 'body', 'target_device_id', 'targetdeviceid'),
      op_param('volumecontainers', 'body', 'volume_containers', 'volumecontainers'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = devices_failover(params)
