...
def managers_get_public_encryption_key(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.StorSimple/managers/%{manager_name}/listPublicEncryptionKey" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('encryptionalgorithm', 'body', 'encryption_algorithm', 'encryptionalgorithm'),
      ...
      op_param('valuecertificatethumbprint', 'body', 'value_certificate_thumbprint', 'valuecertificatethumbprint'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = managers_get_public_encryption_key(params)
