...
def volumes_list_metric_definition(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.StorSimple/managers/%{manager_name}/devices/%{device_name}/volumeContainers/%{volume_container_name}/volumes/%{volume_name}/metricsDefinitions" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('volumeContainerName', 'path', 'volume_container_name', 'volume_container_name'),
      op_param('volumeName', 'path', 'volume_name', 'volume_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = volumes_list_metric_definition(params)
