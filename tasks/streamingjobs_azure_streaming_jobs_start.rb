...
def streaming_jobs_start(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourcegroups/%{resource_group_name}/providers/Microsoft.StreamAnalytics/streamingjobs/%{job_name}/start" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('outputstartmode', 'body', 'output_start_mode', 'outputstartmode'),
      op_param('outputstarttime', 'body', 'output_start_time', 'outputstarttime'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('startJobParameters', 'body', 'start_job_parameters', 'start_job_parameters'),
...
def task
  ...
  params = STDIN.read
  result = streaming_jobs_start(params)
