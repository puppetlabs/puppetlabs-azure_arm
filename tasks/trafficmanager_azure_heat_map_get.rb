...
def heat_map_get(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/trafficmanagerprofiles/%{profile_name}/heatMaps/%{heat_map_type}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('botRight', 'query', 'bot_right', 'bot_right'),
      op_param('error', 'body', 'error', 'error'),
      op_param('heatMapType', 'path', 'heat_map_type', 'heat_map_type'),
      ...
      op_param('topLeft', 'query', 'top_left', 'top_left'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
