...
def delete_image_regions(*args)
  ...
  uri_string = "https://southcentralus.api.cognitive.microsoft.com//projects/%{project_id}/images/regions" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('regionIds', 'query', 'region_ids', 'region_ids'),
...
def authenticate
  ...
  params = STDIN.read
  result = DeleteImageRegions(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = delete_image_regions(params)
