...
def get_image_performances(*args)
  header_params = {}
  ...
  uri_string = "https://southcentralus.api.cognitive.microsoft.com//projects/%{project_id}/iterations/%{iteration_id}/performance/images" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('orderBy', 'query', 'order_by', 'order_by'),
      op_param('projectId', 'path', 'project_id', 'project_id'),
      op_param('skip', 'query', 'skip', 'skip'),
      op_param('tagIds', 'query', 'tag_ids', 'tag_ids'),
      op_param('take', 'query', 'take', 'take'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetImagePerformances(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_image_performances(params)
