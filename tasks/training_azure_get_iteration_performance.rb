...
def get_iteration_performance(*args)
  header_params = {}
  ...
  uri_string = "https://southcentralus.api.cognitive.microsoft.com//projects/%{project_id}/iterations/%{iteration_id}/performance" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('overlapThreshold', 'query', 'overlap_threshold', 'overlap_threshold'),
      op_param('projectId', 'path', 'project_id', 'project_id'),
      op_param('threshold', 'query', 'threshold', 'threshold'),
...
def authenticate
  ...
  params = STDIN.read
  result = GetIterationPerformance(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = get_iteration_performance(params)
