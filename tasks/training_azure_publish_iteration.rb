...
def publish_iteration(*args)
  ...
  uri_string = "https://southcentralus.api.cognitive.microsoft.com//projects/%{project_id}/iterations/%{iteration_id}/publish" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('predictionId', 'query', 'prediction_id', 'prediction_id'),
      op_param('projectId', 'path', 'project_id', 'project_id'),
      op_param('publishName', 'query', 'publish_name', 'publish_name'),
...
def authenticate
  ...
  params = STDIN.read
  result = PublishIteration(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = publish_iteration(params)
