...
def query_predictions(*args)
  ...
  uri_string = "https://southcentralus.api.cognitive.microsoft.com//projects/%{project_id}/predictions/query" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('results', 'body', 'results', 'results'),
      op_param('token', 'body', 'token', 'token'),
...
def authenticate
  ...
  params = STDIN.read
  result = QueryPredictions(params)
  exit 0
...
def task
  # Get operation parameters from an input JSON
  params = STDIN.read
  result = query_predictions(params)
