...
def translator_translate(*args)
  ...
  uri_string = "https://api.cognitive.microsofttranslator.com//translate" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('category', 'query', 'category', 'category'),
      op_param('error', 'body', 'error', 'error'),
      op_param('from', 'query', 'from', 'from'),
      op_param('fromScript', 'query', 'from_script', 'from_script'),
      op_param('includeAlignment', 'query', 'include_alignment', 'include_alignment'),
      op_param('includeSentenceLength', 'query', 'include_sentence_length', 'include_sentence_length'),
      op_param('profanityAction', 'query', 'profanity_action', 'profanity_action'),
      op_param('profanityMarker', 'query', 'profanity_marker', 'profanity_marker'),
      op_param('suggestedFrom', 'query', 'suggested_from', 'suggested_from'),
      op_param('text', 'body', 'text', 'text'),
      op_param('textType', 'query', 'text_type', 'text_type'),
      op_param('to', 'query', 'to', 'to'),
      op_param('toScript', 'query', 'to_script', 'to_script'),
      op_param('X-ClientTraceId', 'header', 'x_client_trace_id', 'x_client_trace_id'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = translator_translate(params)
