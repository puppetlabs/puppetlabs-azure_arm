...
def subscription_usages_get(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/providers/Microsoft.Sql/locations/%{location_name}/usages/%{usage_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('usageName', 'path', 'usage_name', 'usage_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = subscription_usages_get(params)
