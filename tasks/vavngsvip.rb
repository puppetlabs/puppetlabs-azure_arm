...
def virtual_network_gateways_set_vpnclient_ipsec_parameters(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/virtualNetworkGateways/%{virtual_network_gateway_name}/setvpnclientipsecparameters" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('dhgroup', 'body', 'dh_group', 'dhgroup'),
      op_param('ikeencryption', 'body', 'ike_encryption', 'ikeencryption'),
      op_param('ikeintegrity', 'body', 'ike_integrity', 'ikeintegrity'),
      op_param('ipsecencryption', 'body', 'ipsec_encryption', 'ipsecencryption'),
      op_param('ipsecintegrity', 'body', 'ipsec_integrity', 'ipsecintegrity'),
      op_param('pfsgroup', 'body', 'pfs_group', 'pfsgroup'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('sadatasizekilobytes', 'body', 'sa_data_size_kilobytes', 'sadatasizekilobytes'),
      op_param('salifetimeseconds', 'body', 'sa_life_time_seconds', 'salifetimeseconds'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('virtualNetworkGatewayName', 'path', 'virtual_network_gateway_name', 'virtual_network_gateway_name'),
      op_param('vpnclientIpsecParams', 'body', 'vpnclient_ipsec_params', 'vpnclient_ipsec_params'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_network_gateways_set_vpnclient_ipsec_parameters(params)
