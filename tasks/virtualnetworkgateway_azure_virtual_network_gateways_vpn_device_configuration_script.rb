...
def virtual_network_gateways_vpn_device_configuration_script(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/connections/%{virtual_network_gateway_connection_name}/vpndeviceconfigurationscript" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('devicefamily', 'body', 'device_family', 'devicefamily'),
      op_param('firmwareversion', 'body', 'firmware_version', 'firmwareversion'),
      ...
      op_param('vendor', 'body', 'vendor', 'vendor'),
      op_param('virtualNetworkGatewayConnectionName', 'path', 'virtual_network_gateway_connection_name', 'virtual_network_gateway_connection_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_network_gateways_vpn_device_configuration_script(params)
