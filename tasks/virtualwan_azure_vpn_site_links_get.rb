...
def vpn_site_links_get(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/vpnSites/%{vpn_site_name}/vpnSiteLinks/%{vpn_site_link_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('vpnSiteLinkName', 'path', 'name', 'vpn_site_link_name'),
      op_param('vpnSiteName', 'path', 'vpn_site_name', 'vpn_site_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = vpn_site_links_get(params)
