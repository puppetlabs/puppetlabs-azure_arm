...
def vpn_sites_configuration_download(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Network/virtualWans/%{virtual_wan_name}/vpnConfiguration" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('outputblobsasurl', 'body', 'output_blob_sas_url', 'outputblobsasurl'),
      op_param('request', 'body', 'request', 'request'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('virtualWANName', 'path', 'virtual_wan_name', 'virtual_wan_name'),
      op_param('vpnsites', 'body', 'vpn_sites', 'vpnsites'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = vpn_sites_configuration_download(params)
