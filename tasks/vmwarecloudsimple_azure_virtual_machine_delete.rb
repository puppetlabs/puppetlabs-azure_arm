...
def virtual_machine_delete(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.VMwareCloudSimple/virtualMachines/%{virtual_machine_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('virtualMachineName', 'path', 'name', 'virtual_machine_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_machine_delete(params)
