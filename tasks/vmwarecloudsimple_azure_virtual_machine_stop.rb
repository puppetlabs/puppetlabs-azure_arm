...
def virtual_machine_stop(*args)
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.VMwareCloudSimple/virtualMachines/%{virtual_machine_name}/stop" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('Referer', 'header', 'referer', 'referer'),
      op_param('resourceGroupName', 'path', 'resource_group_name', 'resource_group_name'),
      op_param('subscriptionId', 'path', 'subscription_id', 'subscription_id'),
      op_param('virtualMachineName', 'path', 'virtual_machine_name', 'virtual_machine_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = virtual_machine_stop(params)
