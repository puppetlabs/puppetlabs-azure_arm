...
def web_apps_list_publishing_profile_xml_with_secrets_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/publishxml" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('format', 'body', 'format', 'format'),
      op_param('includedisasterrecoveryendpoints', 'body', 'include_disaster_recovery_endpoints', 'includedisasterrecoveryendpoints'),
      op_param('name', 'path', 'name', 'name'),
      op_param('publishingProfileOptions', 'body', 'publishing_profile_options', 'publishing_profile_options'),
...
def task
  ...
  params = STDIN.read
  result = web_apps_list_publishing_profile_xml_with_secrets_slot(params)
