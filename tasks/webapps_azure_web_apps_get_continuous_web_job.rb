...
def web_apps_get_continuous_web_job(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/continuouswebjobs/%{web_job_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('webJobName', 'path', 'web_job_name', 'web_job_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = web_apps_get_continuous_web_job(params)
