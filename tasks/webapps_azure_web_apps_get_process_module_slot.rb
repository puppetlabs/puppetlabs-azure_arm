...
def web_apps_get_process_module_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/processes/%{process_id}/modules/%{base_address}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('baseAddress', 'path', 'base_address', 'base_address'),
      op_param('id', 'body', 'id', 'id'),
      op_param('kind', 'body', 'kind', 'kind'),
      op_param('name', 'path', 'name', 'name'),
      op_param('processId', 'path', 'process_id', 'process_id'),
...
def task
  ...
  params = STDIN.read
  result = web_apps_get_process_module_slot(params)
