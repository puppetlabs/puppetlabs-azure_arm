...
def web_apps_get_process_thread_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/processes/%{process_id}/threads/%{thread_id}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('threadId', 'path', 'thread_id', 'thread_id'),
...
def task
  ...
  params = STDIN.read
  result = web_apps_get_process_thread_slot(params)
