...
def web_apps_get_web_job_slot(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.Web/sites/%{name}/slots/%{slot}/webjobs/%{web_job_name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('webJobName', 'path', 'name', 'web_job_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = web_apps_get_web_job_slot(params)
