...
def web_services_create_regional_properties(*args)
  header_params = {}
  ...
  uri_string = "https://management.azure.com//subscriptions/%{subscription_id}/resourceGroups/%{resource_group_name}/providers/Microsoft.MachineLearning/webServices/%{web_service_name}/CreateRegionalBlob" % path_params
...
def format_params(key_values)
  ...
  op_params = [
      ...
      op_param('errorinfo', 'body', 'error_info', 'errorinfo'),
      op_param('id', 'body', 'id', 'id'),
      op_param('name', 'body', 'name', 'name'),
      op_param('percentcomplete', 'body', 'percent_complete', 'percentcomplete'),
      op_param('provisioningstate', 'body', 'provisioning_state', 'provisioningstate'),
      op_param('region', 'query', 'region', 'region'),
      ...
      op_param('webServiceName', 'path', 'web_service_name', 'web_service_name'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = web_services_create_regional_properties(params)
